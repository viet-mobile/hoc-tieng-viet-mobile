(function () {
  ...
  var I18N_UI = {
    ...
    "차이": {"vi": "Khác biệt", "cs": "Rozdíly", "zh_cn": "差异", "zh": "差異", "en": "Contrasts", "fr": "Différences", "de": "Unterschiede", "hu": "Különbségek", "id": "Perbedaan", "ja": "違い", "pl": "Różnice"},
  ...
  (function () {
    ...
    var SUBTAB_SHORT_NAMES = {
      ...
      "차이": {"de": "Vergleich", "fr": "Écarts", "hu": "Különbség"},
  ...
  // [빈도] (sort by frequency) sits left of the [태그] on/off toggle in the word-count row.
  var WORD_SORT_LABEL = { vi: "Tần suất", cs: "Četnost", zh_cn: "频率", zh: "頻率", en: "Fre\u00adquency", fr: "Fré\u00adquence", de: "Häufig\u00adkeit", hu: "Gyako\u00adriság", id: "Frek\u00aduensi", ja: "頻度", ko: "빈도", pl: "Częs\u00adtość" };
  var WORD_COUNT_FORMAT = { vi: "{n} từ", cs: "{n} slov", zh_cn: "{n} 词", zh: "{n} 詞", en: "{n} words", fr: "{n} mots", de: "{n} Wörter", hu: "{n} szó", id: "{n} kata", ja: "{n} 語", ko: "{n} 단어", pl: "{n} słów" };
  ...
  // WORD_TAG_KEY); the WT/노래/행누 (Enjoy Life Forever, key "elf") source tags from JW_EXTRACTION_DATA occurrence counts.
  var WORD_TAG_DEFS = [
    ...
    { key: "elf", menu: {"vi": "Vui hưởng", "cs": "Užívej", "zh_cn": "享受", "zh": "享受", "en": "Enjoy Life", "fr": "Vivez", "de": "Glück­lich", "hu": "Élvezd", "id": "Nikmati", "ja": "楽しく", "ko": "행누", "pl": "Ciesz się"}, short: {"vi": "V.hưởng", "cs": "Užív.", "zh_cn": "享受", "zh": "享受", "en": "ELF", "fr": "Viv.", "de": "Glück", "hu": "Élv.", "id": "Nikm.", "ja": "楽しく", "ko": "행누", "pl": "Ciesz"} },
  ...
  function renderVocabWords(root, q) {
    ...
    if (filterEl) {
      ...
      // Always two rows: JW profiles' 14 tags -> 7 per row, GENERAL's smaller set -> 5 per row.
    ...
    var html = '<div class="chain-note word-count-row">' +
      ...
      '<button type="button" class="tag-filter-btn word-tag-toggle" aria-pressed="' + (wordsShowTags ? 'true' : 'false') + '">' +
        escapeHtml(WORD_TAG_TOGGLE_LABEL[currentLang] || WORD_TAG_TOGGLE_LABEL.ko) + '</button>' +
  ...
  var PRON_PANE_ELS = {
    ...
    diff: document.getElementById("pron-diff-pane"),
  ...
  // [발음] > [차이]: PRON_COMPARISON (pronunciation_comparison/ at build time) in the UI
  // language. The section comparing Vietnamese with the UI language comes first and starts open;
  // the rest follow in PRON_COMPARISON.order (the order the Vietnamese UI uses as is).
  function pronDiffHtml() {
    if (typeof PRON_COMPARISON === "undefined" || !PRON_COMPARISON || !PRON_COMPARISON.langs) return "";
    var content = PRON_COMPARISON.langs[currentLang] || PRON_COMPARISON.langs.en;
    if (!content) return "";
    var order = PRON_COMPARISON.order.slice();
    if (order.indexOf(currentLang) > 0) order = [currentLang].concat(order.filter(function (id) { return id !== currentLang; }));
    function blocksHtml(blocks) {
      return (blocks || []).map(function (b) {
        if (b.h) return '<h4 class="pron-cmp-h">' + escapeHtml(b.h) + '</h4>';
        if (b.ul) return '<ul class="pron-cmp-list">' + b.ul.map(function (li) { return '<li>' + escapeHtml(li) + '</li>'; }).join("") + '</ul>';
        return '<p class="pron-cmp-p">' + escapeHtml(b.p) + '</p>';
      }).join("");
    }
    function card(extraClass, title, bodyHtml, open) {
      return '<div class="group-card ' + extraClass + '" data-open="' + (open ? "true" : "false") + '">' +
        '<div class="group-head-row"><button class="group-head"><span class="pron-cmp-title">' + escapeHtml(title) + '</span>' + currChev() + '</button></div>' +
    ...
    order.forEach(function (id, i) {
      var sec = content.sections[id];
      if (!sec) return;
      var body = sec.note ? blocksHtml(sec.note) : "";
      if (sec.subsections) {
        body += sec.subsections.map(function (sub) {
          return '<h3 class="pron-cmp-sub">' + escapeHtml(sub.title) + '</h3>' + partsHtml(sub);
        }).join("");
      } else {
        body += partsHtml(sec);
      }
      html += card("pron-cmp-section", sec.title, body, i === 0);
    });
    return html + '</div>';
  }
  function renderPron() {
...
/* END SHARED-DISTRIBUTION-ENGINE */
  ...
  // The [발음] > [설정] TTS cards for this device (same user-agent signals as IS_MOBILE_UA).
  // A Galaxy is recognised only when the user agent says so (Samsung Internet, or an "SM-"
  // model number); Chrome's reduced Android user agent hides the maker, so any other Android
  // device gets both the Samsung Galaxy card (listed first) and the other-Android card.
  function ttsGuideDeviceKeys() {
    var ua = navigator.userAgent || "";
    if (/iPhone|iPad|iPod/i.test(ua) || (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1)) return ["ios"];
    if (/Android/i.test(ua)) return /SamsungBrowser|; SM-[A-Z0-9]/i.test(ua) ? ["samsung"] : ["samsung", "android"];
  ...
  function goToDefaultPlace(openDeviceGuide) {
    ...
    var cards = ttsGuideDeviceKeys().map(function (key) {
      return document.querySelector('#panel-pron .group-card[data-tts="' + key + '"]');
    }).filter(Boolean);
    if (!cards.length) return;
    cards.forEach(function (card) { card.dataset.open = "true"; });
    setTimeout(function () { cards[0].scrollIntoView({ block: "start" }); }, 150);
  }
