(function () {
  ...
  var VALID_LANGS = ["ko", "zh", "en", "ja", "de"];
  ...
  // and English plus every other system language (including ones this app has no UI for) -> en.
  ...
  // A first path segment of /ko, /en, /zt, /ja, or /de (see _redirects, which rewrites each of
  ...
  var PATH_LANG_MAP = { ko: "ko", en: "en", zt: "zh", ja: "ja", de: "de" };
  ...
  function T(field) {
    ...
    return field[currentLang] || field.ko || field.zh || field.en || field.ja || field.de || "";
  }
  ...
  // that has no zh/en/ja/de entry yet (e.g. some LFF_CONVERSATIONS lines are missing 'zh' entirely)
  // would otherwise offer a Korean-language flashcard/multiple-choice option while the UI is in
  // zh/en/ja/de mode. Tstrict() returns null instead of silently falling back to a different
  ...
  function Tstrict(field) {
    ...
    if (currentLang === "ko") return field.ko || field.zh || field.en || field.ja || field.de || null;
  ...
  function setLang(lang) {
    ...
    var htmlLang = lang === "ko" ? "ko" : lang === "zh" ? "zh-Hant" : lang === "ja" ? "ja" : lang === "de" ? "de" : "en";
  ...
  "정답 시 다음 문제": { "zh": "答對後下一題", "en": "Next question when correct", "ja": "正解で次の問題", "de": "Bei richtiger Antwort weiter" },
  "반복 듣기": { "zh": "重複播放", "en": "Repeat", "ja": "繰り返し再生", "de": "Wiederholen" },
  "묵음": { "zh": "靜音", "en": "Mute", "ja": "ミュート", "de": "Stumm" },
  "첫만남": { "zh": "初次見面", "en": "First Meeting", "ja": "初対面", "de": "Erstes Gespräch" },
  "다음 문제": {
    "zh": "下一題",
    "en": "Next question",
    "ja": "次の問題",
  ...
  "문장 종류": { "zh": "句子種類", "en": "Sentence Type", "ja": "文の種類", "de": "Satzart" },
  "의문사 선택": { "zh": "選擇疑問詞", "en": "Choose a Wh-word", "ja": "疑問詞を選択", "de": "Fragewort auswählen" },
  "접속어 (문장 앞에 붙는 연결어)": { "zh": "連接詞（放在句子前面的連接語）", "en": "Connective (linking word before the sentence)", "ja": "接続語（文頭につく接続語）", "de": "Bindewort (vorangestellte Konjunktion)" },
  "사용 안 함": { "zh": "不使用", "en": "Not used", "ja": "使用しない", "de": "Nicht verwendet" },
  "예: Sau khi": { "zh": "例：Sau khi", "en": "e.g. Sau khi", "ja": "例：Sau khi", "de": "z. B. Sau khi" },
  "예: 그 후에": { "zh": "例：之後", "en": "e.g. after that", "ja": "例：その後に", "de": "z. B. danach" },
  "주어 (의문사 'ai(누가)'를 쓸 때처럼 주어가 필요 없으면 '사용 안 함')": { "zh": "主語（像使用疑問詞 'ai(誰)' 時那樣不需要主語時，選「不使用」）", "en": "Subject (choose \"Not used\" when no subject is needed, like when using the wh-word 'ai' (who))", "ja": "主語（疑問詞「ai（誰）」を使うときのように主語が要らない場合は「使用しない」）", "de": "Subjekt (wenn kein Subjekt benötigt wird, wählen Sie „Nicht verwendet“)" },
  "예: học viên mới": { "zh": "例：học viên mới", "en": "e.g. học viên mới", "ja": "例：học viên mới", "de": "z. B. học viên mới" },
  "예: 새 학습자": { "zh": "例：新學習者", "en": "e.g. new student", "ja": "例：新しい学習者", "de": "z. B. neuer Bibelschüler" },
  "보조동사": { "zh": "輔助動詞", "en": "Auxiliary Verb", "ja": "補助動詞", "de": "Hilfsverb / Modalverb" },
  "직접 입력": { "zh": "手動輸入", "en": "Custom input", "ja": "直接入力", "de": "Eigene Eingabe" },
  "예: định": { "zh": "例：định", "en": "e.g. định", "ja": "例：định", "de": "z. B. định" },
  "예: ~할 예정이다": { "zh": "例：打算~", "en": "e.g. plan to ~", "ja": "例：~する予定だ", "de": "z. B. vorhaben zu ~" },
  "뜻 패턴: ~하고 싶다": { "zh": "意思模式：想要~", "en": "Meaning pattern: want to ~", "ja": "意味パターン：~したい", "de": "Bedeutungsmuster: möchten ~" },
  "~해야 하다": { "zh": "必須~", "en": "must ~", "ja": "~しなければならない", "de": "müssen ~" },
  "~할 수 있다": { "zh": "能~", "en": "can ~", "ja": "~することができる", "de": "können ~" },
  "~할 수 없다": { "zh": "不能~", "en": "cannot ~", "ja": "~することができない", "de": "nicht können ~" },
  "~하는 게 좋다": { "zh": "最好~", "en": "should ~", "ja": "~したほうがいい", "de": "sollten ~" },
  "~할 것이다(미래)": { "zh": "將要~（未來）", "en": "will ~ (future)", "ja": "~するだろう（未来）", "de": "werden ~ (Zukunft)" },
  "~했다(과거)": { "zh": "~了（過去）", "en": "~ed (past)", "ja": "~した（過去）", "de": "hat ~t (Vergangenheit)" },
  "~하는 중이다": { "zh": "正在~", "en": "be ~ing", "ja": "~している", "de": "gerade dabei sein zu ~" },
  "~할 필요가 있다": { "zh": "需要~", "en": "need to ~", "ja": "~する必要がある", "de": "brauchen zu ~" },
  "~하는 것을 좋아하다": { "zh": "喜歡~", "en": "like to ~", "ja": "~するのが好き", "de": "gerne tun ~" },
  "~해라(명령)": { "zh": "~吧（命令）", "en": "~! (imperative)", "ja": "~しなさい（命令）", "de": "Mach ~! (Befehl)" },
  "가장 뜻이 비슷한 패턴을 골라야 문장이 올바르게 만들어져요.": { "zh": "要選擇意思最相近的模式，句子才會正確生成。", "en": "Pick the pattern closest in meaning so the sentence is generated correctly.", "ja": "意味が一番近いパターンを選ぶと、正しい文が作られます。", "de": "Wählen Sie das Muster mit der ähnlichsten Bedeutung, damit der Satz korrekt gebildet wird." },
  "동사 (베트남어는 형용사만으로도 문장이 완성돼요. 예: Trời lạnh. 날씨가 춥다.)": { "zh": "動詞（越南語光用形容詞也能完成句子。例：Trời lạnh. 天氣冷。）", "en": "Verb (Vietnamese can form a complete sentence with just an adjective. e.g. Trời lạnh. — It's cold.)", "ja": "動詞（ベトナム語は形容詞だけでも文が完成します。例：Trời lạnh. 天気が寒い。）", "de": "Verb (im Vietnamesischen reicht oft schon ein Adjektiv für einen vollständigen Satz, z. B. Trời lạnh. — Das Wetter ist kalt.)" },
  "사용 안 함 (형용사만으로 문장 완성)": { "zh": "不使用（只用形容詞完成句子）", "en": "Not used (sentence completed with adjective only)", "ja": "使用しない（形容詞だけで文を完成）", "de": "Nicht verwendet (Satz nur mit Adjektiv)" },
  "예: nấu": { "zh": "例：nấu", "en": "e.g. nấu", "ja": "例：nấu", "de": "z. B. nấu" },
  "사전형, 예: 요리하다": { "zh": "辭典形，例：요리하다", "en": "dictionary form, e.g. 요리하다", "ja": "辞書形、例：요리하다", "de": "Grundform, z. B. kochen" },
  "자동사 (목적어 없음)": { "zh": "不及物動詞（無受詞）", "en": "Intransitive (no object)", "ja": "自動詞（目的語なし）", "de": "Intransitiv (kein Objekt)" },
  "타동사 (목적어 있음)": { "zh": "及物動詞（有受詞）", "en": "Transitive (has object)", "ja": "他動詞（目的語あり）", "de": "Transitiv (mit Objekt)" },
  "과거형(선택, 비우면 자동생성) 예: 요리했": { "zh": "過去式（可選，留空則自動生成）例：요리했", "en": "past form (optional, auto-generated if left blank) e.g. 요리했", "ja": "過去形（任意、空欄なら自動生成）例：요리했", "de": "Vergangenheitsform (optional), z. B. kochte" },
  "목적어 (타동사를 골랐을 때만 적용돼요)": { "zh": "受詞（只有選了及物動詞才會套用）", "en": "Object (applies only when a transitive verb is chosen)", "ja": "目的語（他動詞を選んだときのみ適用）", "de": "Objekt (nur bei transitiven Verben wirksam)" },
  "예: bức thư": { "zh": "例：bức thư", "en": "e.g. bức thư", "ja": "例：bức thư", "de": "z. B. bức thư" },
  "예: 편지": { "zh": "例：信", "en": "e.g. letter", "ja": "例：手紙", "de": "z. B. Brief" },
  "명사 (동사로 'là(이다)'를 골랐을 때, 주어를 설명하는 명사예요: 주어+là+명사)": { "zh": "名詞（選了動詞 'là(是)' 時，用來說明主語的名詞：主語+là+名詞）", "en": "Noun (when the verb 'là' (be) is chosen, this noun describes the subject: subject + là + noun)", "ja": "名詞（動詞「là（だ）」を選んだとき、主語を説明する名詞：主語+là+名詞）", "de": "Nomen (wenn als Verb „là“ (sein) gewählt wurde: Subjekt + là + Nomen)" },
  "예: người tiên phong": { "zh": "例：người tiên phong", "en": "e.g. người tiên phong", "ja": "例：người tiên phong", "de": "z. B. người tiên phong" },
  "예: 파이오니아": { "zh": "例：先鋒", "en": "e.g. pioneer", "ja": "例：開拓者", "de": "z. B. Pionier" },
  "전치사 (với 등)": { "zh": "介係詞（với 等）", "en": "Preposition (với etc.)", "ja": "前置詞（với など）", "de": "Präposition (với usw.)" },
  "예: với anh trai": { "zh": "例：với anh trai", "en": "e.g. với anh trai", "ja": "例：với anh trai", "de": "z. B. với anh trai" },
  "예: 형과": { "zh": "例：跟哥哥", "en": "e.g. with older brother", "ja": "例：兄と", "de": "z. B. mit dem älteren Bruder" },
  "형용사 (목적어·명사가 있으면 그것을, 없으면 명사 주어를 꾸며요)": { "zh": "形容詞（若有受詞·名詞就修飾它，沒有的話就修飾名詞主語）", "en": "Adjective (modifies the object/noun if present, otherwise the noun subject)", "ja": "形容詞（目的語・名詞があればそれを、なければ名詞主語を修飾）", "de": "Adjektiv (beschreibt das Objekt/Nomen oder das Subjekt)" },
  "예: vui": { "zh": "例：vui", "en": "e.g. vui", "ja": "例：vui", "de": "z. B. vui" },
  "사전형, 예: 기쁘다": { "zh": "辭典形，例：기쁘다", "en": "dictionary form, e.g. 기쁘다", "ja": "辞書形、例：기쁘다", "de": "Grundform, z. B. erfreut sein" },
  "관형사형(선택) 예: 기쁜": { "zh": "冠形詞形（可選）例：기쁜", "en": "attributive form (optional) e.g. 기쁜", "ja": "連体形（任意）例：기쁜", "de": "Attributiv (optional), z. B. erfreut" },
  "과거형(선택) 예: 기뻤": { "zh": "過去式（可選）例：기뻤", "en": "past form (optional) e.g. 기뻤", "ja": "過去形（任意）例：기뻤", "de": "Vergangenheit (optional), z. B. war erfreut" },
  "상태부사 (어떻게)": { "zh": "狀態副詞（怎麼樣）", "en": "Adverb of manner (how)", "ja": "様態副詞（どのように）", "de": "Modaladverb (wie)" },
  "예: lặng lẽ": { "zh": "例：lặng lẽ", "en": "e.g. lặng lẽ", "ja": "例：lặng lẽ", "de": "z. B. lặng lẽ" },
  "예: 조용히": { "zh": "例：安靜地", "en": "e.g. quietly", "ja": "例：静かに", "de": "z. B. leise" },
  "장소부사 (어디에서·어디로) — 장소 명사만 입력하면 동사에 맞는 ở/đến이 자동으로 붙어요": { "zh": "地點副詞（在哪裡·去哪裡）— 只要輸入地點名詞，系統會依動詞自動加上 ở/đến", "en": "Place adverb (where at / where to) — just enter the place noun; ở/đến is added automatically to match the verb", "ja": "場所副詞（どこで・どこへ）— 場所の名詞だけ入力すれば、動詞に合わせて ở/đến が自動で付きます", "de": "Lokaladverb (wo/wohin) — geben Sie nur das Ortsnomen ein; ở/đến wird passend zum Verb ergänzt" },
  "장소 명사만, 예: thư viện": { "zh": "只要地點名詞，例：thư viện", "en": "place noun only, e.g. thư viện", "ja": "場所の名詞だけ、例：thư viện", "de": "Nur das Ortsnomen, z. B. thư viện" },
  "예: 도서관": { "zh": "例：圖書館", "en": "e.g. library", "ja": "例：図書館", "de": "z. B. Bibliothek" },
  "시간부사 (언제)": { "zh": "時間副詞（何時）", "en": "Adverb of time (when)", "ja": "時間副詞（いつ）", "de": "Temporaladverb (wann)" },
  "예: tuần sau": { "zh": "例：tuần sau", "en": "e.g. tuần sau", "ja": "例：tuần sau", "de": "z. B. tuần sau" },
  "예: 다음 주": { "zh": "例：下週", "en": "e.g. next week", "ja": "例：来週", "de": "z. B. nächste Woche" },
  "자동사": { "zh": "不及物動詞", "en": "Intransitive", "ja": "自動詞", "de": "Intransitiv" },
  "타동사": { "zh": "及物動詞", "en": "Transitive", "ja": "他動詞", "de": "Transitiv" },
  "대명사": { "zh": "代名詞", "en": "Pronoun", "ja": "代名詞", "de": "Pronomen" },
  "명사": { "zh": "名詞", "en": "Noun", "ja": "名詞", "de": "Nomen" },
  "직접 입력한 동사·형용사·보조동사는 활용이 적용되지 않고, 입력한 뜻 그대로 문장에 표시돼요.": { "zh": "手動輸入的動詞·形容詞·輔助動詞不會套用詞形變化，會直接以您輸入的意思顯示在句子裡。", "en": "Custom verbs, adjectives, and auxiliary verbs are not conjugated — they appear in the sentence exactly as the meaning you typed.", "ja": "直接入力した動詞・形容詞・補助動詞には活用が適用されず、入力した意味がそのまま文に表示されます。", "de": "Selbst eingegebene Verben, Adjektive und Modalverben werden nicht gebeugt, sondern genau wie eingegeben im Satz angezeigt." },

  /* ---- 가족 호칭 가계도 (family tree) ---- */
  "가족 호칭 가계도": { "zh": "家族稱謂家譜圖", "en": "Family Address-Term Tree", "ja": "家族の呼び方系図", "de": "Stammbaum der familiären Anredeformen" },
  ...
  "혼인으로 맺어지는 사돈 쪽 호칭": { "zh": "因婚姻而形成的親家稱謂", "en": "In-law terms formed through marriage", "ja": "結婚で結ばれる姻戚の呼び方", "de": "Angeheiratete Verwandtschaft" },
  "증조부모": { "zh": "曾祖父母", "en": "Great-grandparents", "ja": "曾祖父母", "de": "Urgroßeltern" },
  "조부모": { "zh": "祖父母", "en": "Grandparents", "ja": "祖父母", "de": "Großeltern" },
  "부모": { "zh": "父母", "en": "Parents", "ja": "両親", "de": "Eltern" },
  "나·형제자매·배우자": { "zh": "我‧兄弟姊妹‧配偶", "en": "Me, siblings & spouse", "ja": "私・兄弟姉妹・配偶者", "de": "Ich, Geschwister & Ehepartner" },
  "자녀": { "zh": "子女", "en": "Children", "ja": "子ども", "de": "Kinder" },
  "손주": { "zh": "孫子女", "en": "Grandchildren", "ja": "孫", "de": "Enkel" },
  "증손주": { "zh": "曾孫子女", "en": "Great-grandchildren", "ja": "ひ孫", "de": "Urenkel" },
  "친가(아버지 쪽)": { "zh": "父系（父親那邊）", "en": "Paternal side (father's side)", "ja": "父方（父の実家）", "de": "Väterlicherseits" },
  "외가(어머니 쪽)": { "zh": "母系（母親那邊）", "en": "Maternal side (mother's side)", "ja": "母方（母の実家）", "de": "Mütterlicherseits" },
  "형제자매": { "zh": "兄弟姊妹", "en": "Siblings", "ja": "兄弟姉妹", "de": "Geschwister" },
  "배우자": { "zh": "配偶", "en": "Spouse", "ja": "配偶者", "de": "Ehepartner" },
  "며느리·사위": { "zh": "媳婦‧女婿", "en": "Children's spouses", "ja": "嫁・婿", "de": "Schwiegerkinder" },
  "친손(아들의 자녀)": { "zh": "孫（兒子的子女）", "en": "Grandchildren via son", "ja": "息子の子（内孫）", "de": "Enkel (über den Sohn)" },
  "외손(딸의 자녀)": { "zh": "外孫（女兒的子女）", "en": "Grandchildren via daughter", "ja": "娘の子（外孫）", "de": "Enkel (über die Tochter)" },
  "친증손(아들 쪽 손주의 자녀)": { "zh": "曾孫（兒子那邊孫子女的子女）", "en": "Great-grandchildren via son's line", "ja": "息子側のひ孫", "de": "Urenkel (Linie des Sohnes)" },
  "외증손(딸 쪽 손주의 자녀)": { "zh": "外曾孫（女兒那邊孫子女的子女）", "en": "Great-grandchildren via daughter's line", "ja": "娘側のひ孫", "de": "Urenkel (Linie der Tochter)" },
  "배우자의 부모님": { "zh": "配偶的父母", "en": "Spouse's parents", "ja": "配偶者の両親", "de": "Schwiegereltern" },
  "남편의 부모(아내 입장)": { "zh": "丈夫的父母（妻子的立場）", "en": "Husband's parents (from the wife's side)", "ja": "夫の両親（妻の立場）", "de": "Schwiegereltern (aus Sicht der Ehefrau)" },
  "아내의 부모(남편 입장)": { "zh": "妻子的父母（丈夫的立場）", "en": "Wife's parents (from the husband's side)", "ja": "妻の両親（夫の立場）", "de": "Schwiegereltern (aus Sicht des Ehemanns)" },
  "배우자의 형제자매": { "zh": "配偶的兄弟姊妹", "en": "Spouse's siblings", "ja": "配偶者の兄弟姉妹", "de": "Geschwister des Ehepartners" },
  "남편의 형제자매(아내 입장)": { "zh": "丈夫的兄弟姊妹（妻子的立場）", "en": "Husband's siblings (from the wife's side)", "ja": "夫の兄弟姉妹（妻の立場）", "de": "Schwäger/Schwägerinnen (aus Sicht der Ehefrau)" },
  "아내의 형제자매(남편 입장)": { "zh": "妻子的兄弟姊妹（丈夫的立場）", "en": "Wife's siblings (from the husband's side)", "ja": "妻の兄弟姉妹（夫の立場）", "de": "Schwäger/Schwägerinnen (aus Sicht des Ehemanns)" },
  "형제자매의 배우자": { "zh": "兄弟姊妹的配偶", "en": "Siblings' spouses", "ja": "兄弟姉妹の配偶者", "de": "Ehepartner der Geschwister" },
  "오빠·남동생의 아내": { "zh": "哥哥‧弟弟的妻子", "en": "Older/younger brother's wife", "ja": "兄・弟の妻", "de": "Ehefrau des Bruders (Schwägerin)" },
  "언니·여동생의 남편": { "zh": "姊姊‧妹妹的丈夫", "en": "Older/younger sister's husband", "ja": "姉・妹の夫", "de": "Ehemann der Schwester (Schwager)" },
  "자녀의 배우자 쪽 부모": { "zh": "子女配偶的父母", "en": "Children's spouses' parents", "ja": "子どもの配偶者の両親", "de": "Eltern des Schwiegerkindes" },
  "사돈": { "zh": "親家", "en": "Co-parents-in-law", "ja": "サドン（子の配偶者の親同士）", "de": "Gegenschwiegereltern" },
  "증조할아버지": { "zh": "曾祖父", "en": "Great-grandfather (paternal)", "ja": "曾祖父（父方）", "de": "Urgroßvater (väterlicherseits)" },
  "증조할머니": { "zh": "曾祖母", "en": "Great-grandmother (paternal)", "ja": "曾祖母（父方）", "de": "Urgroßmutter (väterlicherseits)" },
  "외증조할아버지": { "zh": "外曾祖父", "en": "Great-grandfather (maternal)", "ja": "曾祖父（母方）", "de": "Urgroßvater (mütterlicherseits)" },
  "외증조할머니": { "zh": "外曾祖母", "en": "Great-grandmother (maternal)", "ja": "曾祖母（母方）", "de": "Urgroßmutter (mütterlicherseits)" },
  "할아버지": { "zh": "爺爺", "en": "Grandfather (paternal)", "ja": "祖父（父方）", "de": "Großvater (väterlicherseits)" },
  "할머니": { "zh": "奶奶", "en": "Grandmother (paternal)", "ja": "祖母（父方）", "de": "Großmutter (väterlicherseits)" },
  "외할아버지": { "zh": "外公", "en": "Grandfather (maternal)", "ja": "祖父（母方）", "de": "Großvater (mütterlicherseits)" },
  "외할머니": { "zh": "外婆", "en": "Grandmother (maternal)", "ja": "祖母（母方）", "de": "Großmutter (mütterlicherseits)" },
  "아버지": { "zh": "父親", "en": "Father", "ja": "父", "de": "Vater" },
  "어머니": { "zh": "母親", "en": "Mother", "ja": "母", "de": "Mutter" },
  "남동생": { "zh": "弟弟", "en": "Younger brother", "ja": "弟", "de": "Jüngerer Bruder" },
  "여동생": { "zh": "妹妹", "en": "Younger sister", "ja": "妹", "de": "Jüngere Schwester" },
  "남편": { "zh": "丈夫", "en": "Husband", "ja": "夫", "de": "Ehemann" },
  "아내": { "zh": "妻子", "en": "Wife", "ja": "妻", "de": "Ehefrau" },
  "아들": { "zh": "兒子", "en": "Son", "ja": "息子", "de": "Sohn" },
  "딸": { "zh": "女兒", "en": "Daughter", "ja": "娘", "de": "Tochter" },
  "며느리": { "zh": "媳婦", "en": "Daughter-in-law", "ja": "嫁（息子の妻）", "de": "Schwiegertochter" },
  "사위": { "zh": "女婿", "en": "Son-in-law", "ja": "婿（娘の夫）", "de": "Schwiegersohn" },
  "친손자": { "zh": "孫子", "en": "Grandson (via son)", "ja": "孫息子（内孫）", "de": "Enkelsohn (vom Sohn)" },
  "친손녀": { "zh": "孫女", "en": "Granddaughter (via son)", "ja": "孫娘（内孫）", "de": "Enkeltochter (vom Sohn)" },
  "외손자": { "zh": "外孫", "en": "Grandson (via daughter)", "ja": "孫息子（外孫）", "de": "Enkelsohn (von der Tochter)" },
  "외손녀": { "zh": "外孫女", "en": "Granddaughter (via daughter)", "ja": "孫娘（外孫）", "de": "Enkeltochter (von der Tochter)" },
  "친증손자": { "zh": "曾孫", "en": "Great-grandson (son's line)", "ja": "ひ孫息子（内系）", "de": "Urenkel (vom Sohn)" },
  "친증손녀": { "zh": "曾孫女", "en": "Great-granddaughter (son's line)", "ja": "ひ孫娘（内系）", "de": "Urenkelin (vom Sohn)" },
  "외증손자": { "zh": "外曾孫", "en": "Great-grandson (daughter's line)", "ja": "ひ孫息子（外系）", "de": "Urenkel (von der Tochter)" },
  "외증손녀": { "zh": "外曾孫女", "en": "Great-granddaughter (daughter's line)", "ja": "ひ孫娘（外系）", "de": "Urenkelin (von der Tochter)" },
  "시아버지(남편의 아버지)": { "zh": "公公（丈夫的父親）", "en": "Father-in-law (husband's father)", "ja": "義父（夫の父）", "de": "Schwiegervater (Vater des Ehemanns)" },
  "시어머니(남편의 어머니)": { "zh": "婆婆（丈夫的母親）", "en": "Mother-in-law (husband's mother)", "ja": "義母（夫の母）", "de": "Schwiegermutter (Mutter des Ehemanns)" },
  "장인(아내의 아버지)": { "zh": "岳父（妻子的父親）", "en": "Father-in-law (wife's father)", "ja": "義父（妻の父）", "de": "Schwiegervater (Vater der Ehefrau)" },
  "장모(아내의 어머니)": { "zh": "岳母（妻子的母親）", "en": "Mother-in-law (wife's mother)", "ja": "義母（妻の母）", "de": "Schwiegermutter (Mutter der Ehefrau)" },
  "아주버님(남편의 형)": { "zh": "大伯（丈夫的哥哥）", "en": "Husband's older brother", "ja": "夫の兄", "de": "Älterer Bruder des Ehemanns (Schwager)" },
  "손위 시누이(남편의 누나)": { "zh": "大姑（丈夫的姊姊）", "en": "Husband's older sister", "ja": "夫の姉", "de": "Ältere Schwester des Ehemanns (Schwägerin)" },
  "시동생(남편의 남동생)": { "zh": "小叔（丈夫的弟弟）", "en": "Husband's younger brother", "ja": "夫の弟", "de": "Jüngerer Bruder des Ehemanns (Schwager)" },
  "손아래 시누이(남편의 여동생)": { "zh": "小姑（丈夫的妹妹）", "en": "Husband's younger sister", "ja": "夫の妹", "de": "Jüngere Schwester des Ehemanns (Schwägerin)" },
  "손위 처남(아내의 오빠)": { "zh": "大舅子（妻子的哥哥）", "en": "Wife's older brother", "ja": "妻の兄", "de": "Älterer Bruder der Ehefrau (Schwager)" },
  "처형(아내의 언니)": { "zh": "大姨子（妻子的姊姊）", "en": "Wife's older sister", "ja": "妻の姉", "de": "Ältere Schwester der Ehefrau (Schwägerin)" },
  "손아래 처남(아내의 남동생)": { "zh": "小舅子（妻子的弟弟）", "en": "Wife's younger brother", "ja": "妻の弟", "de": "Jüngerer Bruder der Ehefrau (Schwager)" },
  "처제(아내의 여동생)": { "zh": "小姨子（妻子的妹妹）", "en": "Wife's younger sister", "ja": "妻の妹", "de": "Jüngere Schwester der Ehefrau (Schwägerin)" },
  "올케(오빠의 아내)": { "zh": "嫂子（哥哥的妻子）", "en": "Older brother's wife", "ja": "兄の妻", "de": "Ehefrau des älteren Bruders" },
  "올케(남동생의 아내)": { "zh": "弟妹（弟弟的妻子）", "en": "Younger brother's wife", "ja": "弟の妻", "de": "Ehefrau des jüngeren Bruders" },
  "형부(언니의 남편)": { "zh": "姊夫（姊姊的丈夫）", "en": "Older sister's husband", "ja": "姉の夫", "de": "Ehemann der älteren Schwester" },
  "제부(여동생의 남편)": { "zh": "妹夫（妹妹的丈夫）", "en": "Younger sister's husband", "ja": "妹の夫", "de": "Ehemann der jüngeren Schwester" },
  "사돈(자녀 배우자의 부모)": { "zh": "親家（子女配偶的父母）", "en": "Co-parents-in-law (child's spouse's parents)", "ja": "サドン（子の配偶者の親）", "de": "Eltern des Schwiegerkindes" },
  "사돈을 맺다": { "zh": "結為親家", "en": "To become co-parents-in-law", "ja": "サドン（姻戚）の関係を結ぶ", "de": "Verschwägert werden" },
  "사돈어른(바깥사돈)": { "zh": "親家公（男方親家）", "en": "Male co-parent-in-law", "ja": "男性側の親家（サドンの夫）", "de": "Schwiegerkindsvater" },
  "안사돈": { "zh": "親家母（女方親家）", "en": "Female co-parent-in-law", "ja": "女性側の親家（サドンの妻）", "de": "Schwiegerkindsmutter" },
  "제3자에게 말할 때": { "zh": "對第三者提起時", "en": "When mentioning them to someone else", "ja": "第三者に話すとき", "de": "Beim Sprechen über Dritte" },
  "직접 호칭할 때": { "zh": "直接稱呼時", "en": "When addressing them directly", "ja": "直接呼びかけるとき", "de": "Bei direkter Anrede" },
  "행복한 삶을 영원히": { "zh": "永遠享受美好的生命", "en": "Enjoy Life Forever", "ja": "いつまでも幸せに暮らせます", "de": "Glücklich – für immer" },
  ...
  "부": { "zh": "部分", "en": "Part", "ja": "部", "de": "Teil" },
  "베트남어 문장이나 뜻으로 검색": { "zh": "用越南語句子或意思搜尋", "en": "Search by Vietnamese sentence or meaning", "ja": "ベトナム語の文や意味で検索", "de": "Nach vietnamesischem Satz oder Bedeutung suchen" },
  "나는 준비가 되었는가?": { "zh": "我準備好了嗎？", "en": "Am I Ready?", "ja": "準備はできていますか", "de": "Bin ich bereit?" },
  "사람들을 사랑하고 제자로": { "zh": "用愛心幫助人成為基督徒", "en": "Love People—Make Disciples", "ja": "愛を込めて弟子を育てる", "de": "Menschen lieben – Jünger machen" },
  ...
  "부록": { "zh": "附錄", "en": "Appendix", "ja": "付録", "de": "Anhang" },
  "노래 선택": { "zh": "選擇詩歌", "en": "Select Song", "ja": "歌の選択", "de": "Lied auswählen" },
  "바둑판": { "zh": "網格", "en": "Grid", "ja": "グリッド", "de": "Kacheln" },
  "목록": { "zh": "列表", "en": "List", "ja": "一覧", "de": "Liste" },
  "곡 번호 또는 제목 검색... (예: 12 또는 사랑)": { "zh": "搜尋歌曲編號或標題... (例: 12 或 愛)", "en": "Search song number or title... (e.g. 12 or Love)", "ja": "番号や曲名で検索... (例: 12 または 愛)", "de": "Liednummer oder Titel suchen... (z. B. 12 oder Liebe)" },
  "검색 결과가 없습니다.": { "zh": "找不到搜尋結果。", "en": "No results found.", "ja": "検索結果がありません。", "de": "Keine Suchergebnisse." },
  "닫기": { "zh": "關閉", "en": "Close", "ja": "閉じる", "de": "Schließen" },
  "전체": { "zh": "全部", "en": "All", "ja": "すべて", "de": "Alle" }
};
  function TU(ko) {
    if (!ko) return ko;
  ...
  // Use short, readable English and German names when primary tabs share a narrow row.
  ...
  (function () {
    ...
    function syncPrimaryTabLabels() {
      var compact = (currentLang === "en" || currentLang === "de") && window.innerWidth < 1000;
  ...
  (function bindLangSwitch() {
    ...
    document.documentElement.setAttribute(
      "lang",
      currentLang === "ko" ? "ko" : currentLang === "zh" ? "zh-Hant" : currentLang === "ja" ? "ja" : currentLang === "de" ? "de" : "en"
    );
