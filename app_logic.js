(function () {
  ...
  "학습 과정": {"vi": "Khóa học", "cs": "Studijní kurz", "zh_cn": "学习课程", "zh": "學習課程", "en": "Study Course", "fr": "Cours d’étude", "de": "Lernkurs", "hu": "Tanulmányi kurzus", "id": "Kursus belajar", "ja": "学習コース", "pl": "Kurs nauki"},
    ...
    "<p class=\"p-desc\">iOS/iPadOS의 내장 기능을 사용하면 별도의 앱 설치 없이 베트남어 텍스트를 읽게 할 수 있어요.</p><h4>1단계: 베트남어 TTS 음성 다운로드하기</h4><ol><li>설정 앱을 엽니다.</li><li><b>손쉬운 사용</b> 메뉴로 이동합니다.</li><li><b>읽기 및 말하기</b>(또는 iOS 버전에 따라 <b>콘텐츠 말하기</b>)를 선택합니다.</li><li><b>음성</b>을 탭합니다.</li><li>스크롤을 내려 <b>베트남어</b>를 선택합니다.</li><li>원하는 목소리(예: Linh) 옆의 구름 모양(다운로드) 아이콘을 눌러 음성 데이터를 다운로드합니다. ‘고품질’ 버전을 다운로드하면 훨씬 자연스러운 AI 음성으로 들을 수 있어요.</li></ol><h4>2단계: 화면의 베트남어 텍스트 읽히기 (사용 방법)</h4><p class=\"p-desc\">다운로드가 완료되면 다음 두 가지 방법으로 베트남어를 읽게 만들 수 있어요. 동일하게 설정 &gt; 손쉬운 사용 &gt; 콘텐츠 말하기 메뉴에서 활성화할 수 있습니다.</p><h4>방법 A: 선택 항목 말하기 (추천)</h4><ol><li>설정 방법: ‘선택 항목 말하기’ 기능을 켭니다.</li><li>사용 방법: 웹서핑이나 텍스트 앱에서 베트남어 단어·문장을 길게 눌러 블록을 지정한 후, 팝업 메뉴에서 ‘말하기’를 누르면 베트남어로 읽어줘요.</li></ol><h4>방법 B: 화면 말하기</h4><ol><li>설정 방법: ‘화면 말하기’ 기능을 켭니다.</li><li>사용 방법: 베트남어로 된 뉴스나 전자책 화면에서 두 손가락으로 화면 상단에서 아래로 쓸어내리면 화면 전체의 텍스트를 자동으로 인식해 처음부터 끝까지 읽어줘요.</li></ol>": {"zh_cn": "<p class=\"p-desc\">ä½¿ç¨ iOS/iPadOS çåå»ºåè½,ä¸éè¦å¦å¤å®è£ App,å°±è½è®©è£ç½®æè¯»è¶åè¯­æå­ã</p><h4>æ­¥é©ä¸:ä¸è½½è¶åè¯­ TTS è¯­é³</h4><ol><li>å¼å¯ãè®¾å®ãAppã</li><li>åå¾<b>è¾å©ä½¿ç¨</b>éåã</li><li>éæ©<b>æè¯»åå®¹</b>(ä¾ iOS çæ¬ä¸å,äº¦å¯è½æ¾ç¤ºä¸º<b>è®²åºåå®¹</b>)ã</li><li>ç¹ä¸ä¸<b>è¯­é³</b>ã</li><li>å¾ä¸æ²å¨å¹¶éæ©<b>è¶åè¯­</b>ã</li><li>ç¹éæ³è¦çè¯­é³(ä¾å¦ Linh)æè¾¹çé²æµ(ä¸è½½)å¾ç¤º,å³å¯ä¸è½½è¯­é³èµæãè¥ä¸è½½ãé«åè´¨ãçæ¬,å°±è½å¬å°æ´èªç¶ç AI è¯­é³ã</li></ol><h4>æ­¥é©äº:è®©è¢å¹ä¸çè¶åè¯­æå­è¢«æè¯»åºæ¥(ä½¿ç¨æ¹æ³)</h4><p class=\"p-desc\">ä¸è½½å®æå,å¯ä»¥éè¿ä»¥ä¸ä¸¤ç§æ¹å¼è®©è£ç½®æè¯»è¶åè¯­ãè¿ä¸¤é¡¹åè½åæ ·å¯å¨ãè®¾å® &gt; è¾å©ä½¿ç¨ &gt; è®²åºåå®¹ãéåä¸­å¼å¯ã</p><h4>æ¹æ³ A:æè¯»æéé¡¹ç®(æ¨è¦)</h4><ol><li>è®¾å®æ¹æ³:å¼å¯ãæè¯»æéé¡¹ç®ãåè½ã</li><li>ä½¿ç¨æ¹æ³:å¨æµè§ç½é¡µææå­ App æ¶,é¿æè¶åè¯­åå­æå¥å­ä»¥éåèå´,æ¥çå¨å½åºå¼éåä¸­ç¹éãæè¯»ã,å³å¯å¬å°è¶åè¯­æè¯»ã</li></ol><h4>æ¹æ³ B:æè¯»è¢å¹</h4><ol><li>è®¾å®æ¹æ³:å¼å¯ãæè¯»è¢å¹ãåè½ã</li><li>ä½¿ç¨æ¹æ³:å¨æ¾ç¤ºè¶åè¯­æ°èæçµå­ä¹¦çç»é¢ä¸,ç¨ä¸¤æ ¹ææä»è¢å¹é ç«¯åä¸æ»å¨,ç³»ç»å°±ä¼èªå¨è¾¨è¯æ´ä¸ªç»é¢çæå­,å¹¶ä»å¤´å°å°¾æè¯»åºæ¥ã</li></ol>", "zh": "<p class=\"p-desc\">ä½¿ç¨ iOS/iPadOS çå§å»ºåè½,ä¸éè¦å¦å¤å®è£ App,å°±è½è®è£ç½®æè®è¶åèªæå­ã</p><h4>æ­¥é©ä¸:ä¸è¼è¶åèª TTS èªé³</h4><ol><li>éåãè¨­å®ãAppã</li><li>åå¾<b>è¼å©ä½¿ç¨</b>é¸å®ã</li><li>é¸æ<b>æè®å§å®¹</b>(ä¾ iOS çæ¬ä¸å,äº¦å¯è½é¡¯ç¤ºçº<b>è¬åºå§å®¹</b>)ã</li><li>é»ä¸ä¸<b>èªé³</b>ã</li><li>å¾ä¸æ²åä¸¦é¸æ<b>è¶åèª</b>ã</li><li>é»é¸æ³è¦çèªé³(ä¾å¦ Linh)æéçé²æµ(ä¸è¼)åç¤º,å³å¯ä¸è¼èªé³è³æãè¥ä¸è¼ãé«åè³ªãçæ¬,å°±è½è½å°æ´èªç¶ç AI èªé³ã</li></ol><h4>æ­¥é©äº:è®è¢å¹ä¸çè¶åèªæå­è¢«æè®åºä¾(ä½¿ç¨æ¹æ³)</h4><p class=\"p-desc\">ä¸è¼å®æå¾,å¯ä»¥ééä»¥ä¸å©ç¨®æ¹å¼è®è£ç½®æè®è¶åèªãéå©é åè½åæ¨£å¯å¨ãè¨­å® &gt; è¼å©ä½¿ç¨ &gt; è¬åºå§å®¹ãé¸å®ä¸­éåã</p><h4>æ¹æ³ A:æè®æé¸é ç®(æ¨è¦)</h4><ol><li>è¨­å®æ¹æ³:éåãæè®æé¸é ç®ãåè½ã</li><li>ä½¿ç¨æ¹æ³:å¨çè¦½ç¶²é ææå­ App æ,é·æè¶åèªå®å­æå¥å­ä»¥é¸åç¯å,æ¥èå¨å½åºå¼é¸å®ä¸­é»é¸ãæè®ã,å³å¯è½å°è¶åèªæè®ã</li></ol><h4>æ¹æ³ B:æè®è¢å¹</h4><ol><li>è¨­å®æ¹æ³:éåãæè®è¢å¹ãåè½ã</li><li>ä½¿ç¨æ¹æ³:å¨é¡¯ç¤ºè¶åèªæ°èæé»å­æ¸çç«é¢ä¸,ç¨å©æ ¹ææå¾è¢å¹é ç«¯åä¸æ»å,ç³»çµ±å°±æèªåè¾¨è­æ´åç«é¢çæå­,ä¸¦å¾é ­å°å°¾æè®åºä¾ã</li></ol>", "en": "<p class=\"p-desc\">You can use the built-in features of iOS/iPadOS to have Vietnamese text read aloud, without installing a separate app.</p><h4>Step 1: Download a Vietnamese TTS Voice</h4><ol><li>Open the Settings app.</li><li>Go to the <b>Accessibility</b> menu.</li><li>Select <b>Spoken Content</b> (or <b>Speak Screen &amp; Content</b>, depending on your iOS version).</li><li>Tap <b>Voices</b>.</li><li>Scroll down and select <b>Vietnamese</b>.</li><li>Tap the cloud (download) icon next to the voice you want (e.g., Linh) to download the voice data. Downloading the \"Premium\" (high-quality) version gives you a much more natural-sounding AI voice.</li></ol><h4>Step 2: Have On-Screen Vietnamese Text Read Aloud (How to Use)</h4><p class=\"p-desc\">Once the download is complete, there are two ways to have Vietnamese read aloud. Both can also be turned on from Settings &gt; Accessibility &gt; Spoken Content.</p><h4>Method A: Speak Selection (Recommended)</h4><ol><li>Setup: Turn on the \"Speak Selection\" feature.</li><li>How to use: While browsing the web or using a text app, press and hold a Vietnamese word or sentence to select it, then tap \"Speak\" in the pop-up menu to hear it read aloud in Vietnamese.</li></ol><h4>Method B: Speak Screen</h4><ol><li>Setup: Turn on the \"Speak Screen\" feature.</li><li>How to use: On a screen showing Vietnamese news or an e-book, swipe down from the top of the screen with two fingers, and all the text on screen will be automatically recognized and read aloud from start to finish.</li></ol>", "fr": "<p class=\"p-desc\">Vous pouvez utiliser les fonctionnalitÃ©s intÃ©grÃ©es d'iOS/iPadOS pour faire lire du texte vietnamien Ã  voix haute sans installer d'application tierce.</p><h4>Ãtape 1 : TÃ©lÃ©charger une voix TTS vietnamienne</h4><ol><li>Ouvrez l'application <b>RÃ©glages</b>.</li><li>Allez dans le menu <b>AccessibilitÃ©</b>.</li><li>SÃ©lectionnez <b>Contenu Ã©noncÃ©</b>.</li><li>Touchez <b>Voix</b>.</li><li>Faites dÃ©filer vers le bas et sÃ©lectionnez <b>Vietnamien</b>.</li><li>Touchez l'icÃ´ne de nuage (tÃ©lÃ©chargement) Ã  cÃ´tÃ© de la voix souhaitÃ©e (ex. : Linh) pour installer les donnÃ©es vocales. La version de haute qualitÃ© offre une voix beaucoup plus naturelle.</li></ol><h4>Ãtape 2 : Faire lire le texte vietnamien Ã  l'Ã©cran</h4><p class=\"p-desc\">Une fois le tÃ©lÃ©chargement terminÃ©, deux options s'offrent Ã  vous :</p><h4>MÃ©thode A : Ãnoncer la sÃ©lection (RecommandÃ©)</h4><ol><li>Activez Â« Ãnoncer la sÃ©lection Â».</li><li>SÃ©lectionnez un mot ou une phrase en vietnamien sur une page web ou dans une application, puis touchez Â« Ãnoncer Â» dans le menu contextuel.</li></ol><h4>MÃ©thode B : Ãnoncer le contenu de l'Ã©cran</h4><ol><li>Activez Â« Ãnoncer le contenu de l'Ã©cran Â».</li><li>Faites glisser deux doigts depuis le haut de l'Ã©cran vers le bas pour lire tout le texte affichÃ©.</li></ol>", "de": "<p class=\"p-desc\">Mit den integrierten Funktionen von iOS/iPadOS kÃ¶nnen Sie sich vietnamesischen Text vorlesen lassen, ohne eine separate App zu installieren.</p><h4>Schritt 1: Vietnamesische TTS-Stimme herunterladen</h4><ol><li>Ãffnen Sie die App <b>Einstellungen</b>.</li><li>Gehen Sie zum MenÃ¼ <b>Bedienungshilfen</b>.</li><li>WÃ¤hlen Sie <b>Gesprochene Inhalte</b>.</li><li>Tippen Sie auf <b>Stimmen</b>.</li><li>Scrollen Sie nach unten und wÃ¤hlen Sie <b>Vietnamesisch</b>.</li><li>Tippen Sie auf das Wolkensymbol (Download) neben der gewÃ¼nschten Stimme (z. B. Linh), um die Sprachdaten herunterzuladen. Die âErweiterteâ Version bietet eine besonders natÃ¼rliche Stimme.</li></ol><h4>Schritt 2: Vietnamesischen Text auf dem Bildschirm vorlesen lassen</h4><p class=\"p-desc\">Nach dem Download gibt es zwei MÃ¶glichkeiten zum Vorlesen. Beide lassen sich unter Einstellungen &gt; Bedienungshilfen &gt; Gesprochene Inhalte aktivieren.</p><h4>Methode A: Auswahl vorlesen (Empfohlen)</h4><ol><li>Einrichtung: Aktivieren Sie âAuswahl vorlesenâ.</li><li>Verwendung: Halten Sie beim Surfen oder in Text-Apps ein vietnamesisches Wort oder einen Satz gedrÃ¼ckt, um den Text auszuwÃ¤hlen, und tippen Sie im MenÃ¼ auf âSprechenâ.</li></ol><h4>Methode B: Bildschirminhalt sprechen</h4><ol><li>Einrichtung: Aktivieren Sie âBildschirminhalt sprechenâ.</li><li>Verwendung: Streichen Sie mit zwei Fingern vom oberen Bildschirmrand nach unten, um den gesamten Text auf dem Bildschirm vorlesen zu lassen.</li></ol>", "ja": "<p class=\"p-desc\">iOS/iPadOSã®åèµæ©è½ãä½¿ãã°ãå¥éã¢ããªãã¤ã³ã¹ãã¼ã«ããªãã¦ããããã èªã®ãã­ã¹ããèª­ã¿ä¸ãããããã¨ãã§ãã¾ãã</p><h4>ã¹ããã1:ãããã èªã®TTSé³å£°ããã¦ã³ã­ã¼ããã</h4><ol><li>ãè¨­å®ãã¢ããªãéãã¾ãã</li><li><b>ã¢ã¯ã»ã·ããªãã£</b>ã¡ãã¥ã¼ã«ç§»åãã¾ãã</li><li><b>èª­ã¿ä¸ãã³ã³ãã³ã</b>(iOSã®ãã¼ã¸ã§ã³ã«ãã£ã¦ã¯<b>ã³ã³ãã³ãã®èª­ã¿ä¸ã</b>)ãé¸æãã¾ãã</li><li><b>å£°</b>ãã¿ãããã¾ãã</li><li>ä¸ã«ã¹ã¯ã­ã¼ã«ãã¦<b>ãããã èª</b>ãé¸æãã¾ãã</li><li>ä½¿ç¨ãããé³å£°(ä¾:Linh)ã®æ¨ªã«ããé²ã®ãã¼ã¯(ãã¦ã³ã­ã¼ã)ã¢ã¤ã³ã³ãã¿ãããã¦é³å£°ãã¼ã¿ããã¦ã³ã­ã¼ããã¾ãããé«åè³ªãçããã¦ã³ã­ã¼ãããã¨ãããèªç¶ãªAIé³å£°ã§èããã¨ãã§ãã¾ãã</li></ol><h4>ã¹ããã2:ç»é¢ä¸ã®ãããã èªãã­ã¹ããèª­ã¿ä¸ãããã(ä½¿ãæ¹)</h4><p class=\"p-desc\">ãã¦ã³ã­ã¼ããå®äºããã¨ãæ¬¡ã®2ã¤ã®æ¹æ³ã§ãããã èªãèª­ã¿ä¸ãããããã¨ãã§ãã¾ããã©ã¡ããè¨­å® &gt; ã¢ã¯ã»ã·ããªãã£ &gt; èª­ã¿ä¸ãã³ã³ãã³ãã®ã¡ãã¥ã¼ããæå¹ã«ã§ãã¾ãã</p><h4>æ¹æ³A:é¸æé ç®ã®èª­ã¿ä¸ã(ãããã)</h4><ol><li>è¨­å®æ¹æ³:ãé¸æé ç®ã®èª­ã¿ä¸ããæ©è½ããªã³ã«ãã¾ãã</li><li>ä½¿ãæ¹:ã¦ã§ããµã¤ãããã­ã¹ãã¢ããªã§ãããã èªã®åèªãæç« ãé·æ¼ããã¦é¸æãããããã¢ããã¡ãã¥ã¼ã§ãèª­ã¿ä¸ãããã¿ããããã¨ããããã èªã§èª­ã¿ä¸ãã¦ããã¾ãã</li></ol><h4>æ¹æ³B:ç»é¢ã®èª­ã¿ä¸ã</h4><ol><li>è¨­å®æ¹æ³:ãç»é¢ã®èª­ã¿ä¸ããæ©è½ããªã³ã«ãã¾ãã</li><li>ä½¿ãæ¹:ãããã èªã®ãã¥ã¼ã¹ãé»å­æ¸ç±ãè¡¨ç¤ºãããç»é¢ã§ã2æ¬æã§ç»é¢ã®ä¸ç«¯ããä¸ã¸ã¹ã¯ã¤ãããã¨ãç»é¢å¨ä½ã®ãã­ã¹ããèªåçã«èªè­ãããæåããæå¾ã¾ã§èª­ã¿ä¸ãããã¾ãã</li></ol>", "pl": "<p class=\"p-desc\">DziÄki wbudowanym funkcjom iOS/iPadOS moÅ¼esz sÅuchaÄ czytanego tekstu wietnamskiego bez instalowania dodatkowych aplikacji.</p><h4>Krok 1: Pobierz gÅos syntezatora mowy (TTS) dla wietnamskiego</h4><ol><li>OtwÃ³rz aplikacjÄ <b>Ustawienia</b>.</li><li>PrzejdÅº do menu <b>DostÄpnoÅÄ</b>.</li><li>Wybierz <b>ZawartoÅÄ mÃ³wiona</b>.</li><li>Dotknij <b>GÅosy</b>.</li><li>PrzewiÅ w dÃ³Å i wybierz <b>Wietnamski</b>.</li><li>Dotknij ikony chmury (pobieranie) obok wybranego gÅosu (np. Linh). Wersja ulepszona zapewnia bardziej naturalny gÅos.</li></ol><h4>Krok 2: Odczytywanie tekstu z ekranu</h4><p class=\"p-desc\">Po pobraniu moÅ¼esz korzystaÄ z dwÃ³ch metod odczytu tekstu:</p><h4>Metoda A: Przeczytaj zaznaczone (Zalecane)</h4><ol><li>WÅÄcz funkcjÄ âPrzeczytaj zaznaczoneâ.</li><li>Zaznacz tekst wietnamski w przeglÄdarce i dotknij âMÃ³wâ w menu podrÄcznym.</li></ol><h4>Metoda B: Przeczytaj ekran</h4><ol><li>WÅÄcz funkcjÄ âPrzeczytaj ekranâ.</li><li>PrzesuÅ dwoma palcami od gÃ³ry ekranu w dÃ³Å, aby odczytaÄ caÅÄ zawartoÅÄ ekranu.</li></ol>"},
    "<h4>1. Google 음성 엔진에서 베트남어 추가 (공통)</h4><p class=\"p-desc\">대부분의 안드로이드 기기에 내장된 구글 엔진에 베트남어 음성 팩을 다운로드하는 방법이에요.</p><ol><li>스마트폰 <b>설정</b> 앱을 엽니다.</li><li><b>접근성</b> ➡️ <b>텍스트 음성 변환 출력</b>(또는 ‘글자 읽어주기’) 메뉴로 이동합니다.</li><li>기본 엔진이 <b>Google 음성 인식 및 합성</b>(또는 Speech Services by Google)으로 선택되어 있는지 확인합니다.</li><li>기본 엔진 옆에 있는 <b>설정(톱니바퀴 아이콘)</b>을 누릅니다.</li><li><b>음성 데이터 설치</b>를 선택합니다.</li><li>목록에서 <b>베트남어</b>(또는 Vietnamese)를 찾아 다운로드 버튼을 누릅니다.</li><li>다운로드가 완료되면 뒤로 돌아와 <b>언어</b>를 <b>베트남어</b>로 지정합니다.</li></ol><h4>2. 삼성 갤럭시 기기에서 베트남어 추가</h4><p class=\"p-desc\">삼성 갤럭시 스마트폰을 사용 중이라면 삼성 전용 고품질 베트남어 TTS를 추가할 수 있어요.</p><ol><li>스마트폰 <b>설정</b> 앱을 엽니다.</li><li><b>일반</b> ➡️ <b>글자 읽어주기</b>(또는 텍스트 음성 변환) 메뉴로 이동합니다.</li><li>기본 엔진을 <b>삼성 TTS 엔진</b>으로 설정합니다.</li><li>엔진 옆의 <b>설정(톱니바퀴 아이콘)</b>을 누릅니다.</li><li><b>음성 데이터 설치</b>를 누르고 목록에서 <b>베트남어</b>를 찾아 다운로드합니다.</li></ol><div class=\"tts-tip\">※ 갤럭시 스토어에서 직접 Samsung TTS Vietnamese Voice 팩을 검색해 설치할 수도 있어요.</div>": {"zh_cn": "<h4>1. å¨ Google è¯­é³å¼æä¸­æ°å¢è¶åè¯­(éç¨)</h4><p class=\"p-desc\">å¤§å¤æ° Android è£ç½®åå»ºç Google å¼æé½å¯ä»¥ä¸è½½è¶åè¯­è¯­é³å,æ¹æ³å¦ä¸ã</p><ol><li>å¼å¯ææºç<b>è®¾å®</b> Appã</li><li>åå¾<b>åå©å·¥å·</b> â¡ï¸ <b>æå­è½¬è¯­é³è¾åº</b>(æç§°ãè¯»åºæå­ã)éåã</li><li>ç¡®è®¤é¢è®¾å¼ææ¯å¦ä¸º<b>Google è¯­é³è¾¨è¯ä¸åæ</b>(æ Speech Services by Google)ã</li><li>ç¹éé¢è®¾å¼ææç<b>è®¾å®(é½è¼ªå¾ç¤º)</b>ã</li><li>éæ©<b>å®è£è¯­é³èµæ</b>ã</li><li>å¨æ¸åä¸­æ¾å°<b>è¶åè¯­</b>(Vietnamese)å¹¶ç¹éä¸è½½æé®ã</li><li>ä¸è½½å®æåè¿åä¸ä¸é¡µ,å°<b>è¯­è¨</b>è®¾å®ä¸º<b>è¶åè¯­</b>ã</li></ol><h4>2. å¨ä¸æ Galaxy è£ç½®ä¸­æ°å¢è¶åè¯­</h4><p class=\"p-desc\">å¦ææ¨ä½¿ç¨çæ¯ä¸æ Galaxy ææº,å¯ä»¥é¡å¤æ°å¢ä¸æå°å±çé«åè´¨è¶åè¯­ TTSã</p><ol><li>å¼å¯ææºç<b>è®¾å®</b> Appã</li><li>åå¾<b>ä¸è¬</b> â¡ï¸ <b>è¯»åºæå­</b>(ææå­è½¬è¯­é³)éåã</li><li>å°é¢è®¾å¼æè®¾å®ä¸º<b>ä¸æ TTS å¼æ</b>ã</li><li>ç¹éå¼ææç<b>è®¾å®(é½è¼ªå¾ç¤º)</b>ã</li><li>ç¹é<b>å®è£è¯­é³èµæ</b>,å¨æ¸åä¸­æ¾å°<b>è¶åè¯­</b>å¹¶ä¸è½½ã</li></ol><div class=\"tts-tip\">â» æ¨ä¹å¯ä»¥ç´æ¥å¨ Galaxy Store ä¸­æå¯» Samsung TTS Vietnamese Voice è¯­é³åå¹¶å®è£ã</div>", "zh": "<h4>1. å¨ Google èªé³å¼æä¸­æ°å¢è¶åèª(éç¨)</h4><p class=\"p-desc\">å¤§å¤æ¸ Android è£ç½®å§å»ºç Google å¼æé½å¯ä»¥ä¸è¼è¶åèªèªé³å,æ¹æ³å¦ä¸ã</p><ol><li>éåææ©ç<b>è¨­å®</b> Appã</li><li>åå¾<b>åå©å·¥å·</b> â¡ï¸ <b>æå­è½èªé³è¼¸åº</b>(æç¨±ãè®åºæå­ã)é¸å®ã</li><li>ç¢ºèªé è¨­å¼ææ¯å¦çº<b>Google èªé³è¾¨è­èåæ</b>(æ Speech Services by Google)ã</li><li>é»é¸é è¨­å¼ææç<b>è¨­å®(é½è¼ªåç¤º)</b>ã</li><li>é¸æ<b>å®è£èªé³è³æ</b>ã</li><li>å¨æ¸å®ä¸­æ¾å°<b>è¶åèª</b>(Vietnamese)ä¸¦é»é¸ä¸è¼æéã</li><li>ä¸è¼å®æå¾è¿åä¸ä¸é ,å°<b>èªè¨</b>è¨­å®çº<b>è¶åèª</b>ã</li></ol><h4>2. å¨ä¸æ Galaxy è£ç½®ä¸­æ°å¢è¶åèª</h4><p class=\"p-desc\">å¦ææ¨ä½¿ç¨çæ¯ä¸æ Galaxy ææ©,å¯ä»¥é¡å¤æ°å¢ä¸æå°å±¬çé«åè³ªè¶åèª TTSã</p><ol><li>éåææ©ç<b>è¨­å®</b> Appã</li><li>åå¾<b>ä¸è¬</b> â¡ï¸ <b>è®åºæå­</b>(ææå­è½èªé³)é¸å®ã</li><li>å°é è¨­å¼æè¨­å®çº<b>ä¸æ TTS å¼æ</b>ã</li><li>é»é¸å¼ææç<b>è¨­å®(é½è¼ªåç¤º)</b>ã</li><li>é»é¸<b>å®è£èªé³è³æ</b>,å¨æ¸å®ä¸­æ¾å°<b>è¶åèª</b>ä¸¦ä¸è¼ã</li></ol><div class=\"tts-tip\">â» æ¨ä¹å¯ä»¥ç´æ¥å¨ Galaxy Store ä¸­æå° Samsung TTS Vietnamese Voice èªé³åä¸¦å®è£ã</div>", "en": "<h4>1. Add Vietnamese in the Google Speech Engine (Common)</h4><p class=\"p-desc\">Here's how to download the Vietnamese voice pack for the Google engine built into most Android devices.</p><ol><li>Open the <b>Settings</b> app on your phone.</li><li>Go to <b>Accessibility</b> â¡ï¸ <b>Text-to-speech output</b> menu.</li><li>Check that the default engine is set to <b>Google Speech Recognition and Synthesis</b> (or Speech Services by Google).</li><li>Tap the <b>Settings (gear icon)</b> next to the default engine.</li><li>Select <b>Install voice data</b>.</li><li>Find <b>Vietnamese</b> in the list and tap the download button.</li><li>Once the download finishes, go back and set the <b>Language</b> to <b>Vietnamese</b>.</li></ol><h4>2. Add Vietnamese on a Samsung Galaxy Device</h4><p class=\"p-desc\">If you're using a Samsung Galaxy phone, you can add Samsung's own high-quality Vietnamese TTS.</p><ol><li>Open the <b>Settings</b> app on your phone.</li><li>Go to <b>General</b> â¡ï¸ <b>Text-to-speech</b> (or Read out text) menu.</li><li>Set the default engine to <b>Samsung TTS engine</b>.</li><li>Tap <b>Settings (gear icon)</b> next to the engine.</li><li>Tap <b>Install voice data</b> and find <b>Vietnamese</b> in the list to download it.</li></ol><div class=\"tts-tip\">â» You can also search for and install the Samsung TTS Vietnamese Voice pack directly from the Galaxy Store.</div>", "fr": "<h4>1. Ajouter le vietnamien dans le moteur vocal Google (Android en gÃ©nÃ©ral)</h4><p class=\"p-desc\">Voici comment tÃ©lÃ©charger le pack vocal vietnamien pour le moteur Google intÃ©grÃ© Ã  la plupart des appareils Android.</p><ol><li>Ouvrez l'application <b>ParamÃ¨tres</b> de votre tÃ©lÃ©phone.</li><li>Allez dans <b>AccessibilitÃ©</b> â¡ï¸ <b>Sortie de synthÃ¨se vocale</b>.</li><li>VÃ©rifiez que le moteur prÃ©fÃ©rÃ© est <b>Reconnaissance et synthÃ¨se vocales Google</b>.</li><li>Appuyez sur l'icÃ´ne d'engrenage (paramÃ¨tres) Ã  cÃ´tÃ© du moteur.</li><li>SÃ©lectionnez <b>Installer les donnÃ©es vocales</b>.</li><li>Recherchez <b>Vietnamien</b> dans la liste et appuyez sur TÃ©lÃ©charger.</li><li>Une fois tÃ©lÃ©chargÃ©, revenez en arriÃ¨re et rÃ©glez la langue sur <b>Vietnamien</b>.</li></ol><h4>2. Ajouter le vietnamien sur les appareils Samsung Galaxy</h4><p class=\"p-desc\">Si vous utilisez un smartphone Samsung Galaxy, vous pouvez installer la voix TTS vietnamienne haute dÃ©finition de Samsung.</p><ol><li>Ouvrez <b>ParamÃ¨tres</b>.</li><li>Allez dans <b>Gestion globale</b> â¡ï¸ <b>SynthÃ¨se vocale</b>.</li><li>DÃ©finissez le moteur prÃ©fÃ©rÃ© sur <b>Moteur de synthÃ¨se vocale Samsung</b>.</li><li>Appuyez sur l'engrenage Ã  cÃ´tÃ© du moteur.</li><li>Appuyez sur <b>Installer les donnÃ©es vocales</b> et tÃ©lÃ©chargez le vietnamien.</li></ol>", "de": "<h4>1. Vietnamesisch zur Google-Sprachausgabe hinzufÃ¼gen (Allgemein)</h4><p class=\"p-desc\">So laden Sie das vietnamesische Sprachpaket fÃ¼r die Google-Engine auf den meisten Android-GerÃ¤ten herunter.</p><ol><li>Ãffnen Sie die App <b>Einstellungen</b>.</li><li>Gehen Sie zu <b>Bedienungshilfen</b> â¡ï¸ <b>Text-in-Sprache-Ausgabe</b>.</li><li>Stellen Sie sicher, dass als bevorzugte Engine <b>Google Sprachausgabe</b> eingestellt ist.</li><li>Tippen Sie auf das <b>Zahnrad-Symbol</b> neben der Engine.</li><li>WÃ¤hlen Sie <b>Sprachdaten installieren</b>.</li><li>Suchen Sie in der Liste nach <b>Vietnamesisch</b> und tippen Sie auf Herunterladen.</li><li>Gehen Sie nach dem Download zurÃ¼ck und wÃ¤hlen Sie als Sprache <b>Vietnamesisch</b>.</li></ol><h4>2. Vietnamesisch auf Samsung-Galaxy-GerÃ¤ten hinzufÃ¼gen</h4><p class=\"p-desc\">Auf Samsung Galaxy-Smartphones kÃ¶nnen Sie die hochwertige Samsung-eigene vietnamesische TTS-Stimme hinzufÃ¼gen.</p><ol><li>Ãffnen Sie <b>Einstellungen</b>.</li><li>Gehen Sie zu <b>Allgemeine Verwaltung</b> â¡ï¸ <b>Text-zu-Sprache</b>.</li><li>WÃ¤hlen Sie als bevorzugte Engine <b>Samsung Text-zu-Sprache-Engine</b>.</li><li>Tippen Sie auf das <b>Zahnrad-Symbol</b> neben der Engine.</li><li>Tippen Sie auf <b>Sprachdaten installieren</b> und laden Sie <b>Vietnamesisch</b> herunter.</li></ol><div class=\"tts-tip\">â» Sie kÃ¶nnen das Sprachpaket âSamsung TTS Vietnamese Voiceâ auch direkt im Galaxy Store suchen und installieren.</div>", "ja": "<h4>1. Googleã®é³å£°ã¨ã³ã¸ã³ã«ãããã èªãè¿½å (å±é)</h4><p class=\"p-desc\">ã»ã¨ãã©ã®Androidç«¯æ«ã«åèµããã¦ããGoogleã¨ã³ã¸ã³ã«ãããã èªã®é³å£°ããã¯ããã¦ã³ã­ã¼ãããæ¹æ³ã§ãã</p><ol><li>ã¹ãã¼ããã©ã³ã®<b>è¨­å®</b>ã¢ããªãéãã¾ãã</li><li><b>ã¦ã¼ã¶ã¼è£å©</b> â¡ï¸ <b>ãã­ã¹ãèª­ã¿ä¸ãã®åºå</b>(ã¾ãã¯ãèª­ã¿ä¸ãæ©è½ã)ã¡ãã¥ã¼ã«ç§»åãã¾ãã</li><li>ããã©ã«ãã¨ã³ã¸ã³ã<b>Googleé³å£°èªè­ã¨åæ</b>(ã¾ãã¯ Speech Services by Google)ã«è¨­å®ããã¦ãããç¢ºèªãã¾ãã</li><li>ããã©ã«ãã¨ã³ã¸ã³ã®æ¨ªã«ãã<b>è¨­å®(æ­¯è»ã¢ã¤ã³ã³)</b>ãã¿ãããã¾ãã</li><li><b>é³å£°ãã¼ã¿ã®ã¤ã³ã¹ãã¼ã«</b>ãé¸æãã¾ãã</li><li>ãªã¹ããã<b>ãããã èª</b>(Vietnamese)ãæ¢ãã¦ãã¦ã³ã­ã¼ããã¿ã³ãã¿ãããã¾ãã</li><li>ãã¦ã³ã­ã¼ããå®äºãããæ»ãã<b>è¨èª</b>ã<b>ãããã èª</b>ã«è¨­å®ãã¾ãã</li></ol><h4>2. Samsung Galaxyç«¯æ«ã§ãããã èªãè¿½å ãã</h4><p class=\"p-desc\">Samsung Galaxyã¹ãã¼ããã©ã³ããä½¿ãã®å ´åãSamsungç¬èªã®é«åè³ªãªãããã èªTTSãè¿½å ã§ãã¾ãã</p><ol><li>ã¹ãã¼ããã©ã³ã®<b>è¨­å®</b>ã¢ããªãéãã¾ãã</li><li><b>ä¸è¬</b> â¡ï¸ <b>èª­ã¿ä¸ãæ©è½</b>(ã¾ãã¯ãã­ã¹ãèª­ã¿ä¸ã)ã¡ãã¥ã¼ã«ç§»åãã¾ãã</li><li>ããã©ã«ãã¨ã³ã¸ã³ã<b>Samsung TTSã¨ã³ã¸ã³</b>ã«è¨­å®ãã¾ãã</li><li>ã¨ã³ã¸ã³ã®æ¨ªã«ãã<b>è¨­å®(æ­¯è»ã¢ã¤ã³ã³)</b>ãã¿ãããã¾ãã</li><li><b>é³å£°ãã¼ã¿ã®ã¤ã³ã¹ãã¼ã«</b>ãã¿ãããããªã¹ããã<b>ãããã èª</b>ãæ¢ãã¦ãã¦ã³ã­ã¼ããã¾ãã</li></ol><div class=\"tts-tip\">â» Galaxy Storeã§ç´æ¥Samsung TTS Vietnamese Voiceããã¯ãæ¤ç´¢ãã¦ã¤ã³ã¹ãã¼ã«ãããã¨ãã§ãã¾ãã</div>", "pl": "<h4>1. Dodawanie wietnamskiego w syntezatorze mowy Google (Android)</h4><p class=\"p-desc\">Instrukcja pobierania pakietu mowy wietnamskiej dla silnika Google.</p><ol><li>OtwÃ³rz <b>Ustawienia</b> w telefonie.</li><li>PrzejdÅº do <b>UÅatwienia dostÄpu</b> â¡ï¸ <b>Zamiana tekstu na mowÄ</b>.</li><li>Upewnij siÄ, Å¼e preferowanym mechanizmem sÄ <b>UsÅugi mowy Google</b>.</li><li>Dotknij ikony koÅa zÄbatego obok mechanizmu.</li><li>Wybierz <b>Zainstaluj dane gÅosowe</b>.</li><li>ZnajdÅº <b>Wietnamski</b> i pobierz go.</li></ol><h4>2. Dodawanie wietnamskiego na urzÄdzeniach Samsung Galaxy</h4><p class=\"p-desc\">Na telefonach Samsung Galaxy moÅ¼esz zainstalowaÄ wysokiej jakoÅci gÅos Samsung.</p><ol><li>OtwÃ³rz <b>Ustawienia</b> â¡ï¸ <b>ZarzÄdzanie ogÃ³lne</b> â¡ï¸ <b>Zamiana tekstu na mowÄ</b>.</li><li>Ustaw preferowany mechanizm na <b>Mechanizm zamiany tekstu na mowÄ Samsung</b>.</li><li>Dotknij koÅa zÄbatego i zainstaluj dane gÅosowe dla jÄzyka wietnamskiego.</li></ol>"},
    "<p class=\"p-desc\">별도의 프로그램 설치 없이 Mac 시스템 설정에서 바로 다운로드할 수 있어요.</p><h4>1단계: 베트남어 TTS 음성 추가하기</h4><ol><li>Mac 화면 왼쪽 상단의 Apple 메뉴( ) &gt; <b>시스템 설정</b>을 선택합니다.</li><li>사이드바에서 <b>손쉬운 사용</b>을 클릭한 뒤, <b>읽기 및 말하기</b>(또는 macOS 버전에 따라 <b>콘텐츠 말하기</b>)를 선택합니다.</li><li>‘시스템 음성’ 오른쪽에 있는 ⓘ(정보) 버튼 또는 팝업 메뉴를 클릭합니다.</li><li>왼쪽 언어 목록에서 <b>베트남어(Vietnamese)</b>를 찾아 선택합니다.</li><li>원하는 음성(예: Linh 등) 옆의 구름 모양 다운로드 아이콘을 클릭하여 설치합니다.</li><li>다운로드가 완료되면 <b>완료</b>(또는 승인)를 누릅니다.</li></ol><h4>2단계: 시스템 말하기 언어 변경하기</h4><ol><li>‘읽기 및 말하기’ 설정 화면으로 돌아와 ‘시스템 말하기 언어’ 팝업 메뉴를 베트남어로 변경합니다.</li><li>‘시스템 음성’ 메뉴에서 방금 다운로드한 베트남어 음성을 지정합니다.</li><li>아래의 ‘선택 항목 말하기’ 토글을 켜서 활성화합니다.</li></ol><h4>3단계: 단축키로 베트남어 텍스트 읽기</h4><ol><li>웹페이지나 문서에서 베트남어 텍스트를 마우스로 드래그하여 블록 지정(선택)합니다.</li><li>키보드 단축키인 <b>Option + Esc</b>를 동시에 누르면 선택한 베트남어가 원어민 발음으로 재생돼요. (중단하고 싶을 때 다시 누르면 멈춰요.)</li></ol>": {"zh_cn": "<p class=\"p-desc\">ä¸éè¦å¦å¤å®è£ç¨å¼ï¼ç´æ¥å¨ Mac çç³»ç»è®¾å®ä¸­å³å¯ä¸è½½ã</p><h4>ç¬¬1æ­¥ï¼æ°å¢è¶åè¯­ TTS è¯­é³</h4><ol><li>ç¹é Mac ç»é¢å·¦ä¸è§ç Apple éå( ) &gt; <b>ç³»ç»è®¾å®</b>ã</li><li>å¨å´è¾¹æ ç¹é<b>è¾å©ä½¿ç¨</b>ï¼æ¥çéæ©<b>æè¯»åå®¹</b>(ä¾ macOS çæ¬ä¸åï¼å¯è½æ¾ç¤ºä¸º<b>è¯­é³</b>)ã</li><li>ç¹éãç³»ç»è¯­é³ãå³å´ç â(èµè®¯)æé®æå½åºå¼éåã</li><li>å¨å·¦å´è¯­è¨æ¸åä¸­æ¾å°å¹¶éæ©<b>è¶åè¯­(Vietnamese)</b>ã</li><li>ç¹éæ³è¦çè¯­é³(ä¾å¦ Linh ç­)æçé²æµä¸è½½å¾ç¤ºè¿è¡å®è£ã</li><li>ä¸è½½å®æåæä¸<b>å®æ</b>(ææ ¸å)ã</li></ol><h4>ç¬¬2æ­¥ï¼åæ´ç³»ç»æè¯»è¯­è¨</h4><ol><li>è¿åãæè¯»åå®¹ãè®¾å®ç»é¢ï¼å°ãç³»ç»æè¯»è¯­è¨ãå½åºå¼éååæ´ä¸ºè¶åè¯­ã</li><li>å¨ãç³»ç»è¯­é³ãéåä¸­ï¼æå®ååä¸è½½çè¶åè¯­è¯­é³ã</li><li>å¼å¯ä¸æ¹çãæè¯»æéèå´ãåæ¢å¼å³ä»¥å¯ç¨æ­¤åè½ã</li></ol><h4>ç¬¬3æ­¥ï¼ä½¿ç¨å¿«éé®æè¯»è¶åè¯­æå­</h4><ol><li>å¨ç½é¡µææä»¶ä¸­ç¨æ»é¼ ææ³éåè¶åè¯­æå­ã</li><li>åæ¶æä¸é®çå¿«éé®<b>Option + Esc</b>ï¼å³å¯ä»¥æ¯è¯­åé³æè¯»æéçè¶åè¯­æå­ã(æ³è¦åæ­¢æ¶åæä¸æ¬¡å³å¯åæ­¢ã)</li></ol>", "zh": "<p class=\"p-desc\">ä¸éè¦å¦å¤å®è£ç¨å¼ï¼ç´æ¥å¨ Mac çç³»çµ±è¨­å®ä¸­å³å¯ä¸è¼ã</p><h4>ç¬¬1æ­¥ï¼æ°å¢è¶åèª TTS èªé³</h4><ol><li>é»é¸ Mac ç«é¢å·¦ä¸è§ç Apple é¸å®( ) &gt; <b>ç³»çµ±è¨­å®</b>ã</li><li>å¨å´éæ¬é»é¸<b>è¼å©ä½¿ç¨</b>ï¼æ¥èé¸æ<b>æè®å§å®¹</b>(ä¾ macOS çæ¬ä¸åï¼å¯è½é¡¯ç¤ºçº<b>èªé³</b>)ã</li><li>é»é¸ãç³»çµ±èªé³ãå³å´ç â(è³è¨)æéæå½åºå¼é¸å®ã</li><li>å¨å·¦å´èªè¨æ¸å®ä¸­æ¾å°ä¸¦é¸æ<b>è¶åèª(Vietnamese)</b>ã</li><li>é»é¸æ³è¦çèªé³(ä¾å¦ Linh ç­)æçé²æµä¸è¼åç¤ºé²è¡å®è£ã</li><li>ä¸è¼å®æå¾æä¸<b>å®æ</b>(ææ ¸å)ã</li></ol><h4>ç¬¬2æ­¥ï¼è®æ´ç³»çµ±æè®èªè¨</h4><ol><li>è¿åãæè®å§å®¹ãè¨­å®ç«é¢ï¼å°ãç³»çµ±æè®èªè¨ãå½åºå¼é¸å®è®æ´çºè¶åèªã</li><li>å¨ãç³»çµ±èªé³ãé¸å®ä¸­ï¼æå®ååä¸è¼çè¶åèªèªé³ã</li><li>éåä¸æ¹çãæè®æé¸ç¯åãåæééä»¥åç¨æ­¤åè½ã</li></ol><h4>ç¬¬3æ­¥ï¼ä½¿ç¨å¿«ééµæè®è¶åèªæå­</h4><ol><li>å¨ç¶²é ææä»¶ä¸­ç¨æ»é¼ ææ³é¸åè¶åèªæå­ã</li><li>åææä¸éµç¤å¿«ééµ<b>Option + Esc</b>ï¼å³å¯ä»¥æ¯èªç¼é³æè®æé¸çè¶åèªæå­ã(æ³è¦åæ­¢æåæä¸æ¬¡å³å¯åæ­¢ã)</li></ol>", "en": "<p class=\"p-desc\">No separate app needs to be installed â you can download it directly from Mac System Settings.</p><h4>Step 1: Add a Vietnamese TTS Voice</h4><ol><li>Click the Apple menu ( ) at the top-left of your Mac screen &gt; select <b>System Settings</b>.</li><li>Click <b>Accessibility</b> in the sidebar, then select <b>Spoken Content</b> (or <b>Speech</b>, depending on your macOS version).</li><li>Click the â (info) button or pop-up menu next to \"System Voice.\"</li><li>Find and select <b>Vietnamese</b> in the language list on the left.</li><li>Click the cloud download icon next to the voice you want (e.g., Linh) to install it.</li><li>Once the download finishes, click <b>Done</b> (or Approve).</li></ol><h4>Step 2: Change the System Speaking Language</h4><ol><li>Return to the Spoken Content settings screen and change the \"System voice\" language pop-up menu to Vietnamese.</li><li>In the \"System Voice\" menu, select the Vietnamese voice you just downloaded.</li><li>Turn on the \"Speak Selection\" toggle below to enable it.</li></ol><h4>Step 3: Use a Keyboard Shortcut to Read Vietnamese Text Aloud</h4><ol><li>Drag with your mouse to select (highlight) Vietnamese text on a webpage or document.</li><li>Press the keyboard shortcut <b>Option + Esc</b> at the same time to hear the selected Vietnamese text read aloud in a native pronunciation. (Press it again to stop.)</li></ol>", "fr": "<p class=\"p-desc\">Vous pouvez tÃ©lÃ©charger les voix directement dans les RÃ©glages SystÃ¨me de votre Mac sans logiciel tiers.</p><h4>Ãtape 1 : Ajouter une voix TTS vietnamienne</h4><ol><li>Cliquez sur le menu Apple ( ) &gt; <b>RÃ©glages SystÃ¨me</b>.</li><li>Dans la barre latÃ©rale, cliquez sur <b>AccessibilitÃ©</b> puis sur <b>Contenu Ã©noncÃ©</b>.</li><li>Cliquez sur le bouton â ou le menu dÃ©roulant Ã  cÃ´tÃ© de Â« Voix du systÃ¨me Â».</li><li>SÃ©lectionnez <b>Vietnamien</b> dans la liste des langues Ã  gauche.</li><li>Cliquez sur l'icÃ´ne de nuage Ã  cÃ´tÃ© de la voix souhaitÃ©e (ex. : Linh) pour l'installer.</li><li>Cliquez sur <b>TerminÃ©</b>.</li></ol><h4>Ãtape 2 : Configurer la voix du systÃ¨me</h4><ol><li>RÃ©glez la voix du systÃ¨me sur la voix vietnamienne tÃ©lÃ©chargÃ©e.</li><li>Activez l'option Â« Ãnoncer la sÃ©lection Â».</li></ol><h4>Ãtape 3 : Raccourci clavier pour la lecture</h4><ol><li>SÃ©lectionnez du texte vietnamien avec votre souris.</li><li>Appuyez sur <b>Option + Ãchap</b> pour lancer la lecture vocale. Appuyez Ã  nouveau pour arrÃªter.</li></ol>", "de": "<p class=\"p-desc\">Sie kÃ¶nnen vietnamesische Stimmen direkt in den Mac-Systemeinstellungen herunterladen, ohne zusÃ¤tzliche Software zu installieren.</p><h4>Schritt 1: Vietnamesische TTS-Stimme hinzufÃ¼gen</h4><ol><li>WÃ¤hlen Sie das Apple-MenÃ¼ ( ) &gt; <b>Systemeinstellungen</b>.</li><li>Klicken Sie in der Seitenleiste auf <b>Bedienungshilfen</b> und dann auf <b>Gesprochene Inhalte</b>.</li><li>Klicken Sie auf das â-Symbol oder das MenÃ¼ neben âSystemstimmeâ.</li><li>WÃ¤hlen Sie in der Sprachenliste <b>Vietnamesisch</b> aus.</li><li>Klicken Sie auf das Download-Symbol neben der gewÃ¼nschten Stimme (z. B. Linh), um sie zu installieren.</li><li>Klicken Sie nach Abschluss auf <b>Fertig</b>.</li></ol><h4>Schritt 2: Sprachausgabe konfigurieren</h4><ol><li>WÃ¤hlen Sie als Systemstimme die heruntergeladene vietnamesische Stimme aus.</li><li>Aktivieren Sie die Option âAuswahl sprechenâ.</li></ol><h4>Schritt 3: Text per Kurzbefehl vorlesen</h4><ol><li>Markieren Sie vietnamesischen Text auf einer Webseite oder in einem Dokument.</li><li>DrÃ¼cken Sie <b>Wahltaste (Option) + Esc</b>, um den Text anzuhÃ¶ren. Ein erneuter Tastendruck stoppt die Wiedergabe.</li></ol>", "ja": "<p class=\"p-desc\">å¥éã¢ããªãã¤ã³ã¹ãã¼ã«ããå¿è¦ã¯ãªããMacã®ãã·ã¹ãã è¨­å®ãããç´æ¥ãã¦ã³ã­ã¼ãã§ãã¾ãã</p><h4>ã¹ããã1ï¼ãããã èªã®TTSé³å£°ãè¿½å ãã</h4><ol><li>Macç»é¢å·¦ä¸ã®Appleã¡ãã¥ã¼( )&gt;<b>ã·ã¹ãã è¨­å®</b>ãé¸æãã¾ãã</li><li>ãµã¤ããã¼ã§<b>ã¢ã¯ã»ã·ããªãã£</b>ãã¯ãªãã¯ãã<b>èª­ã¿ä¸ãã³ã³ãã³ã</b>(macOSã®ãã¼ã¸ã§ã³ã«ãã£ã¦ã¯<b>ã¹ãã¼ã</b>)ãé¸æãã¾ãã</li><li>ãã·ã¹ãã ã®å£°ãã®å³ã«ãã â(æå ±)ãã¿ã³ã¾ãã¯ãããã¢ããã¡ãã¥ã¼ãã¯ãªãã¯ãã¾ãã</li><li>å·¦å´ã®è¨èªãªã¹ããã<b>ãããã èª(Vietnamese)</b>ãæ¢ãã¦é¸æãã¾ãã</li><li>ä½¿ç¨ãããé³å£°(ä¾ï¼Linhãªã©)ã®æ¨ªã«ããé²å½¢ã®ãã¦ã³ã­ã¼ãã¢ã¤ã³ã³ãã¯ãªãã¯ãã¦ã¤ã³ã¹ãã¼ã«ãã¾ãã</li><li>ãã¦ã³ã­ã¼ããå®äºããã<b>å®äº</b>(ã¾ãã¯æ¿èª)ãã¯ãªãã¯ãã¾ãã</li></ol><h4>ã¹ããã2ï¼ã·ã¹ãã ã®èª­ã¿ä¸ãè¨èªãå¤æ´ãã</h4><ol><li>ãèª­ã¿ä¸ãã³ã³ãã³ããã®è¨­å®ç»é¢ã«æ»ãããã·ã¹ãã ã®èª­ã¿ä¸ãè¨èªããããã¢ããã¡ãã¥ã¼ããããã èªã«å¤æ´ãã¾ãã</li><li>ãã·ã¹ãã ã®å£°ãã¡ãã¥ã¼ã§ãåã»ã©ãã¦ã³ã­ã¼ããããããã èªã®é³å£°ãæå®ãã¾ãã</li><li>ä¸ã«ãããé¸æé ç®ãèª­ã¿ä¸ãããã®ãã°ã«ããªã³ã«ãã¦æå¹ã«ãã¾ãã</li></ol><h4>ã¹ããã3ï¼ã·ã§ã¼ãã«ããã­ã¼ã§ãããã èªã®ãã­ã¹ããèª­ã¿ä¸ãã</h4><ol><li>Webãã¼ã¸ãææ¸ã§ãããã èªã®ãã­ã¹ãããã¦ã¹ã§ãã©ãã°ãã¦é¸æãã¾ãã</li><li>ã­ã¼ãã¼ãã·ã§ã¼ãã«ãã<b>Option + Esc</b>ãåæã«æ¼ãã¨ãé¸æãããããã èªããã¤ãã£ãçºé³ã§åçããã¾ãã(åæ­¢ãããã¨ãã¯ããä¸åº¦æ¼ãã¨æ­¢ã¾ãã¾ãã)</li></ol>", "pl": "<p class=\"p-desc\">MoÅ¼esz pobraÄ gÅosy wietnamskie bezpoÅrednio w Ustawieniach systemowych komputera Mac.</p><h4>Krok 1: Dodaj gÅos wietnamski</h4><ol><li>Wybierz menu Apple ï£¿ &gt; <b>Ustawienia systemowe</b>.</li><li>Kliknij <b>DostÄpnoÅÄ</b>, a nastÄpnie <b>ZawartoÅÄ mÃ³wiona</b>.</li><li>Wybierz jÄzyk <b>Wietnamski</b> i pobierz preferowany gÅos.</li><li>WÅÄcz funkcjÄ âMÃ³w zaznaczoneâ.</li><li>Zaznacz tekst wietnamski i naciÅnij skrÃ³t <b>Option + Esc</b>, aby odsÅuchaÄ wymowÄ.</li></ol>"},
    "<h4>⚙️ Windows 11에서 베트남어 TTS 추가하기</h4><ol><li>설정 열기: 키보드에서 <b>Win + I</b> 단축키를 누릅니다.</li><li>음성 메뉴 이동: 왼쪽 메뉴에서 <b>시간 및 언어</b> ➔ 오른쪽에서 <b>음성</b>을 클릭합니다.</li><li>음성 추가: ‘음성 관리’ 항목에 있는 <b>음성 추가</b> 버튼을 누릅니다.</li><li>베트남어 설치: 검색창에 베트남어를 검색해 선택한 후, <b>추가</b>(또는 설치) 버튼을 누르면 다운로드가 시작돼요.</li><li>기본 음성 설정: 다운로드가 끝나면 상단의 ‘음성 선택’ 드롭다운 메뉴에서 설치된 베트남어 음성을 기본값으로 지정할 수 있어요.</li></ol><h4>⚙️ Windows 10에서 베트남어 TTS 추가하기</h4><ol><li>설정 열기: <b>Win + I</b> 단축키를 누릅니다.</li><li>언어 메뉴 이동: <b>시간 및 언어</b> ➔ 왼쪽의 <b>지역 및 언어</b>(또는 언어) 메뉴를 클릭합니다.</li><li>기본 설정 언어 추가: ‘기본 설정 언어’ 아래의 <b>언어 추가</b>를 클릭합니다.</li><li>베트남어 팩 선택: 베트남어(Tiếng Việt)를 검색하여 선택합니다. 이때 반드시 TTS 아이콘(마이크·말풍선 모양)이 포함되어 있는지 확인하고 ‘다음’을 누릅니다.</li><li>TTS 기능 설치: 기본 기능인 ‘텍스트 음성 변환(TTS)’이 체크된 상태로 <b>설치</b>를 누르면 완료돼요.</li></ol><h4>💡 설치 후 팁 및 확인 사항</h4><div class=\"tts-tip\">설정 반영: 음성 팩 설치를 마친 후에는 사용 중이던 텍스트 리더 프로그램이나 설정 앱을 종료 후 다시 실행해야 베트남어 음성이 정상적으로 표시돼요.</div><div class=\"tts-tip\">간편하게 읽기 실행: 윈도우 기본 돋보기 기능(Win + +)을 켜고 Ctrl + Alt + 마우스 좌클릭을 활용하면 원하는 베트남어 텍스트를 바로 TTS 음성으로 들을 수 있어요. 다만 아쉽게도 윈도우에서 지원하는 “자연스러운 음성” 목록에는 베트남어가 없어서, 윈도우를 사용할 경우 베트남어 TTS 음성이 좀 부자연스럽게 느껴질 수 있어요.</div>": {"zh_cn": "<h4>âï¸ å¨ Windows 11 æ°å¢è¶åè¯­ TTS è¯­é³</h4><ol><li>å¼å¯è®¾å®ï¼å¨é®çä¸æä¸ <b>Win + I</b> å¿«æ·é®ã</li><li>åå¾è¯­é³éåï¼å¨å·¦å´éåç¹é <b>æ¶é´ä¸è¯­è¨</b> â åäºå³å´ç¹é <b>è¯­é³</b>ã</li><li>æ°å¢è¯­é³ï¼å¨âè¯­é³ç®¡çâé¡¹ç®ä¸­æä¸ <b>æ°å¢è¯­é³</b> æé®ã</li><li>å®è£è¶åè¯­ï¼å¨æå¯»æ æå¯»è¶åè¯­å¹¶éååï¼æä¸ <b>æ°å¢</b>ï¼æå®è£ï¼æé®å³å¯å¼å§ä¸è½½ã</li><li>è®¾å®ä¸ºé¢è®¾è¯­é³ï¼ä¸è½½å®æåï¼å¯å¨ä¸æ¹âéæ©è¯­é³âä¸æéåä¸­ï¼å°å·²å®è£çè¶åè¯­è¯­é³è®¾ä¸ºé¢è®¾å¼ã</li></ol><h4>âï¸ å¨ Windows 10 æ°å¢è¶åè¯­ TTS è¯­é³</h4><ol><li>å¼å¯è®¾å®ï¼æä¸ <b>Win + I</b> å¿«æ·é®ã</li><li>åå¾è¯­è¨éåï¼ç¹é <b>æ¶é´ä¸è¯­è¨</b> â å·¦å´ç <b>å°åºä¸è¯­è¨</b>ï¼æè¯­è¨ï¼éåã</li><li>æ°å¢æ¯ç¨è¯­è¨ï¼å¨âæ¯ç¨è¯­è¨âä¸æ¹ç¹é <b>æ°å¢è¯­è¨</b>ã</li><li>éæ©è¶åè¯­è¯­è¨å¥ä»¶ï¼æå¯»å¹¶éåè¶åè¯­ï¼Tiáº¿ng Viá»tï¼ãæ­¤æ¶è¯·å¡å¿ç¡®è®¤å·²åå« TTS å¾ç¤ºï¼éº¥åé£â§è¯­é³æ¡å¾æ¡ï¼ï¼åæä¸âä¸ä¸æ­¥âã</li><li>å®è£ TTS åè½ï¼å¨é¢è®¾å¾éâæå­è½¬è¯­é³ï¼TTSï¼âåè½çç¶æä¸æä¸ <b>å®è£</b> å³å®æã</li></ol><h4>ð¡ å®è£åçæç¤ºä¸ç¡®è®¤äºé¡¹</h4><div class=\"tts-tip\">å¥ç¨è®¾å®ï¼å®æè¯­é³å¥ä»¶å®è£åï¼é¡»åå³é­åéæ°å¼å¯åæ¬ä½¿ç¨ä¸­çæå­éè¯»ç¨å¼æè®¾å®åºç¨ç¨å¼ï¼è¶åè¯­è¯­é³æä¼æ­£å¸¸æ¾ç¤ºã</div><div class=\"tts-tip\">è½»é¬æè¯»ï¼å¼å¯ Windows åå»ºçæ¾å¤§é¡åè½ï¼Win + +ï¼ï¼å¹¶æ­é Ctrl + Alt + æ»é¼ å·¦é®ç¹å»ï¼å³å¯ç«å³ä»¥ TTS è¯­é³æè¯»æéçè¶åè¯­æå­ãä¸è¿å¯æçæ¯ï¼Windows æ¯æ´çâèªç¶è¯­é³âæ¸åä¸­å¹¶æ²¡æè¶åè¯­ï¼å æ­¤å¨ Windows ä¸ä½¿ç¨æ¶ï¼è¶åè¯­ TTS è¯­é³å¯è½ä¼å¬èµ·æ¥ç¥æ¾ä¸èªç¶ã</div>", "zh": "<h4>âï¸ å¨ Windows 11 æ°å¢è¶åèª TTS èªé³</h4><ol><li>éåè¨­å®ï¼å¨éµç¤ä¸æä¸ <b>Win + I</b> å¿«æ·éµã</li><li>åå¾èªé³é¸å®ï¼å¨å·¦å´é¸å®é»é¸ <b>æéèèªè¨</b> â åæ¼å³å´é»é¸ <b>èªé³</b>ã</li><li>æ°å¢èªé³ï¼å¨âèªé³ç®¡çâé ç®ä¸­æä¸ <b>æ°å¢èªé³</b> æéã</li><li>å®è£è¶åèªï¼å¨æå°æ¬æå°è¶åèªä¸¦é¸åå¾ï¼æä¸ <b>æ°å¢</b>ï¼æå®è£ï¼æéå³å¯éå§ä¸è¼ã</li><li>è¨­å®çºé è¨­èªé³ï¼ä¸è¼å®æå¾ï¼å¯å¨ä¸æ¹âé¸æèªé³âä¸æé¸å®ä¸­ï¼å°å·²å®è£çè¶åèªèªé³è¨­çºé è¨­å¼ã</li></ol><h4>âï¸ å¨ Windows 10 æ°å¢è¶åèª TTS èªé³</h4><ol><li>éåè¨­å®ï¼æä¸ <b>Win + I</b> å¿«æ·éµã</li><li>åå¾èªè¨é¸å®ï¼é»é¸ <b>æéèèªè¨</b> â å·¦å´ç <b>å°åèèªè¨</b>ï¼æèªè¨ï¼é¸å®ã</li><li>æ°å¢æ£ç¨èªè¨ï¼å¨âæ£ç¨èªè¨âä¸æ¹é»é¸ <b>æ°å¢èªè¨</b>ã</li><li>é¸æè¶åèªèªè¨å¥ä»¶ï¼æå°ä¸¦é¸åè¶åèªï¼Tiáº¿ng Viá»tï¼ãæ­¤æè«åå¿ç¢ºèªå·²åå« TTS åç¤ºï¼éº¥åé¢¨â§èªé³æ¡åæ¡ï¼ï¼åæä¸âä¸ä¸æ­¥âã</li><li>å®è£ TTS åè½ï¼å¨é è¨­å¾é¸âæå­è½èªé³ï¼TTSï¼âåè½ççæä¸æä¸ <b>å®è£</b> å³å®æã</li></ol><h4>ð¡ å®è£å¾çæç¤ºèç¢ºèªäºé </h4><div class=\"tts-tip\">å¥ç¨è¨­å®ï¼å®æèªé³å¥ä»¶å®è£å¾ï¼é åééåéæ°éååæ¬ä½¿ç¨ä¸­çæå­é±è®ç¨å¼æè¨­å®æç¨ç¨å¼ï¼è¶åèªèªé³æææ­£å¸¸é¡¯ç¤ºã</div><div class=\"tts-tip\">è¼é¬æè®ï¼éå Windows å§å»ºçæ¾å¤§é¡åè½ï¼Win + +ï¼ï¼ä¸¦æ­é Ctrl + Alt + æ»é¼ å·¦éµé»æï¼å³å¯ç«å³ä»¥ TTS èªé³æè®æé¸çè¶åèªæå­ãä¸éå¯æçæ¯ï¼Windows æ¯æ´çâèªç¶èªé³âæ¸å®ä¸­ä¸¦æ²æè¶åèªï¼å æ­¤å¨ Windows ä¸ä½¿ç¨æï¼è¶åèª TTS èªé³å¯è½æè½èµ·ä¾ç¥é¡¯ä¸èªç¶ã</div>", "en": "<h4>âï¸ Adding Vietnamese TTS Voices in Windows 11</h4><ol><li>Open Settings: Press <b>Win + I</b> on your keyboard.</li><li>Go to the Speech menu: In the left menu, click <b>Time & language</b> â then click <b>Speech</b> on the right.</li><li>Add a voice: Under âManage voices,â click the <b>Add voices</b> button.</li><li>Install Vietnamese: Search for Vietnamese in the search box, select it, then click <b>Add</b> (or Install) to start the download.</li><li>Set the default voice: Once the download finishes, you can set the installed Vietnamese voice as the default in the âChoose a voiceâ dropdown menu at the top.</li></ol><h4>âï¸ Adding Vietnamese TTS Voices in Windows 10</h4><ol><li>Open Settings: Press <b>Win + I</b>.</li><li>Go to the language menu: Click <b>Time & language</b> â then <b>Region & language</b> (or Language) on the left.</li><li>Add a preferred language: Under âPreferred languages,â click <b>Add a language</b>.</li><li>Choose the Vietnamese language pack: Search for and select Vietnamese (Tiáº¿ng Viá»t). Make sure the TTS icon (a microphone/speech-bubble icon) is included, then click âNext.â</li><li>Install the TTS feature: With the default âText-to-Speechâ feature checked, click <b>Install</b> to finish.</li></ol><h4>ð¡ Tips and Things to Check After Installation</h4><div class=\"tts-tip\">Apply the settings: After installing a voice pack, close and reopen whatever text-reader program or Settings app you were using â the Vietnamese voice will only show up correctly after that.</div><div class=\"tts-tip\">Quick reading shortcut: Turn on Windowsâ built-in Magnifier (Win + +), then hold Ctrl + Alt and left-click your mouse to instantly hear the selected Vietnamese text read aloud in TTS. Unfortunately, Vietnamese isnât included in the âNatural voicesâ list Windows supports, so Vietnamese TTS voices on Windows may sound a bit less natural.</div>", "fr": "<h4>⚙️ Ajouter la synthèse vocale vietnamienne sous Windows 11</h4><ol><li>Ouvrez les Paramètres : appuyez sur <b>Win + I</b>.</li><li>Allez dans <b>Heure et langue</b> ➔ <b>Voix</b>.</li><li>Sous « Gérer les voix », cliquez sur <b>Ajouter des voix</b>.</li><li>Recherchez le vietnamien et cliquez sur <b>Ajouter</b>.</li><li>Définissez la voix par défaut dans le menu déroulant en haut.</li></ol><h4>⚙️ Ajouter la synthèse vocale vietnamienne sous Windows 10</h4><ol><li>Appuyez sur <b>Win + I</b>.</li><li>Allez dans <b>Heure et langue</b> ➔ <b>Région et langue</b>.</li><li>Cliquez sur <b>Ajouter une langue</b> sous « Langues préférées ».</li><li>Sélectionnez le vietnamien (avec l'icône TTS) et installez-le.</li></ol>", "de": "<h4>âï¸ Vietnamesische TTS unter Windows 11 hinzufÃ¼gen</h4><ol><li>Einstellungen Ã¶ffnen: DrÃ¼cken Sie <b>Win + I</b>.</li><li>Zum SprachmenÃ¼: Klicken Sie auf <b>Zeit und Sprache</b> â <b>Sprachausgabe</b>.</li><li>Stimme hinzufÃ¼gen: Klicken Sie unter âStimmen verwaltenâ auf <b>Stimmen hinzufÃ¼gen</b>.</li><li>Vietnamesisch installieren: Suchen Sie nach Vietnamesisch und klicken Sie auf <b>HinzufÃ¼gen</b>.</li><li>Standardstimme festlegen: WÃ¤hlen Sie oben die installierte vietnamesische Stimme aus.</li></ol><h4>âï¸ Vietnamesische TTS unter Windows 10 hinzufÃ¼gen</h4><ol><li>Einstellungen Ã¶ffnen: DrÃ¼cken Sie <b>Win + I</b>.</li><li>Klicken Sie auf <b>Zeit und Sprache</b> â <b>Region und Sprache</b>.</li><li>Klicken Sie unter âBevorzugte Sprachenâ auf <b>Sprache hinzufÃ¼gen</b>.</li><li>WÃ¤hlen Sie Vietnamesisch (mit TTS-Symbol) aus und klicken Sie auf <b>Installieren</b>.</li></ol><h4>ð¡ Hinweise nach der Installation</h4><div class=\"tts-tip\">Einstellungen Ã¼bernehmen: Starten Sie den Browser oder die Sprach-App nach der Installation neu.</div><div class=\"tts-tip\">Schnellvorlesefunktion: Mit der Windows-Bildschirmlupe (Win + +) und Strg + Alt + Linksklick kÃ¶nnen Sie markierten Text direkt vorlesen lassen.</div>", "ja": "<h4>âï¸ Windows 11ã§ãããã èªã®TTSé³å£°ãè¿½å ãã</h4><ol><li>è¨­å®ãéãï¼ã­ã¼ãã¼ãã§ <b>Win + I</b> ãæ¼ãã¾ãã</li><li>é³å£°ã¡ãã¥ã¼ã¸ç§»åï¼å·¦å´ã®ã¡ãã¥ã¼ã§ <b>æå»ã¨è¨èª</b> â å³å´ã® <b>é³å£°</b> ãã¯ãªãã¯ãã¾ãã</li><li>é³å£°ãè¿½å ï¼âé³å£°ã®ç®¡çâé ç®ã«ãã <b>é³å£°ã®è¿½å </b> ãã¿ã³ãæ¼ãã¾ãã</li><li>ãããã èªãã¤ã³ã¹ãã¼ã«ï¼æ¤ç´¢æ¬ã§ãããã èªãæ¤ç´¢ãã¦é¸æããå¾ã<b>è¿½å </b>ï¼ã¾ãã¯ã¤ã³ã¹ãã¼ã«ï¼ãã¿ã³ãæ¼ãã¨ãã¦ã³ã­ã¼ããå§ã¾ãã¾ãã</li><li>æ¢å®ã®é³å£°ã«è¨­å®ï¼ãã¦ã³ã­ã¼ããå®äºããããä¸é¨ã®âé³å£°ãé¸æâãã­ãããã¦ã³ã¡ãã¥ã¼ã§ã¤ã³ã¹ãã¼ã«ãããããã èªé³å£°ãæ¢å®ã«è¨­å®ã§ãã¾ãã</li></ol><h4>âï¸ Windows 10ã§ãããã èªã®TTSé³å£°ãè¿½å ãã</h4><ol><li>è¨­å®ãéãï¼<b>Win + I</b> ãæ¼ãã¾ãã</li><li>è¨èªã¡ãã¥ã¼ã¸ç§»åï¼<b>æå»ã¨è¨èª</b> â å·¦å´ã® <b>å°åã¨è¨èª</b>ï¼ã¾ãã¯è¨èªï¼ã¡ãã¥ã¼ãã¯ãªãã¯ãã¾ãã</li><li>åªåããè¨èªãè¿½å ï¼âåªåããè¨èªâã®ä¸ã«ãã <b>è¨èªãè¿½å ãã</b> ãã¯ãªãã¯ãã¾ãã</li><li>ãããã èªããã¯ãé¸æï¼ãããã èªï¼Tiáº¿ng Viá»tï¼ãæ¤ç´¢ãã¦é¸æãã¾ãããã®éãå¿ãTTSã¢ã¤ã³ã³ï¼ãã¤ã¯ã»å¹ãåºãã®å½¢ï¼ãå«ã¾ãã¦ãããã¨ãç¢ºèªããâæ¬¡ã¸âãæ¼ãã¾ãã</li><li>TTSæ©è½ãã¤ã³ã¹ãã¼ã«ï¼æ¢å®ã§ãã§ãã¯ããã¦ããâé³å£°åæ(TTS)âæ©è½ããã®ã¾ã¾ã« <b>ã¤ã³ã¹ãã¼ã«</b> ãæ¼ãã°å®äºã§ãã</li></ol><h4>ð¡ ã¤ã³ã¹ãã¼ã«å¾ã®ãã³ãã¨ç¢ºèªäºé </h4><div class=\"tts-tip\">è¨­å®ã®åæ ï¼é³å£°ããã¯ã®ã¤ã³ã¹ãã¼ã«ãå®äºããå¾ã¯ãä½¿ç¨ãã¦ãããã­ã¹ãèª­ã¿ä¸ããã­ã°ã©ã ãè¨­å®ã¢ããªãä¸åº¦çµäºãã¦åèµ·åããªãã¨ããããã èªé³å£°ãæ­£ããè¡¨ç¤ºããã¾ããã</div><div class=\"tts-tip\">æè»½ã«èª­ã¿ä¸ããï¼Windowsæ¨æºã®æ¡å¤§é¡æ©è½ï¼Win + +ï¼ããªã³ã«ããCtrl + Alt ãæ¼ããªãããã¦ã¹ã§å·¦ã¯ãªãã¯ããã¨ãé¸æãããããã èªã®ãã­ã¹ããããã«TTSé³å£°ã§èããã¨ãã§ãã¾ãããã ãæ®å¿µãªãããWindowsããµãã¼ãããâèªç¶ãªé³å£°âãªã¹ãã«ã¯ãããã èªãå«ã¾ãã¦ããªããããWindowsä¸ã§ã¯ãããã èªã®TTSé³å£°ãããä¸èªç¶ã«èããããã¨ãããã¾ãã</div>", "pl": "<h4>⚙️ Dodawanie głosu wietnamskiego w systemie Windows</h4><ol><li>Naciśnij <b>Win + I</b>, aby otworzyć Ustawienia.</li><li>Przejdź do <b>Czas i język</b> ➔ <b>Mowa</b>.</li><li>W sekcji „Zarządzaj głosami” kliknij <b>Dodaj głosy</b> i wyszukaj język wietnamski.</li><li>Po zainstalowaniu wybierz pobrany głos jako domyślny.</li></ol>"},
